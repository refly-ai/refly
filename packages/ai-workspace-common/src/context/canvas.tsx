...
const getInternalState = ({
  ...
  // const nodeLookup = nodeLookup ?? new Map<string, InternalNode>();
...
export const CanvasProvider = ({
  ...
  const setCanvasDataFromYDoc = useCallback(
    (ydoc: Y.Doc) => {
      // setNodes(getNodesFromYDoc(ydoc));
      // setEdges(getEdgesFromYDoc(ydoc));
  ...
  useEffect(() => {
    ...
    const handleConnect = () => {
      ...
      const nodesObserverCallback = () => {
        if (provider.status === 'connected') {
          // setNodes(getNodesFromYDoc(ydoc) as unknown as Node[]);
      ...
      const edgesObserverCallback = () => {
        if (provider.status === 'connected') {
          // setEdges(getEdgesFromYDoc(ydoc) as unknown as Edge[]);
