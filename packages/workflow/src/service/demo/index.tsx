...
const Service: FC = () => {
  const { data: appList, error: appListError } = useSWR(
    { url: "/apps", params: { page: 1 } },
    fetchAppList,
  )
  const { data: firstApp, error: appDetailError } = useSWR(
    { url: "/apps", id: "1" },
    fetchAppDetail,
  )
  const { data: updateAppSiteStatusRes, error: err1 } = useSWR(
    { url: "/apps", id: "1", body: { enable_site: false } },
    updateAppSiteStatus,
  )
  const { data: updateAppApiStatusRes, error: err2 } = useSWR(
    { url: "/apps", id: "1", body: { enable_api: true } },
    updateAppApiStatus,
  )
  const { data: updateAppRateLimitRes, error: err3 } = useSWR(
    { url: "/apps", id: "1", body: { api_rpm: 10, api_rph: 20 } },
    updateAppRateLimit,
  )
  const { data: updateAppSiteCodeRes, error: err4 } = useSWR(
    { url: "/apps", id: "1", body: {} },
    updateAppSiteAccessToken,
  )
  const { data: updateAppSiteConfigRes, error: err5 } = useSWR(
    {
      url: "/apps",
      id: "1",
      body: { title: "title test", author: "author test" },
    },
    updateAppSiteConfig,
  )
  const { data: getAppDailyConversationsRes, error: err6 } = useSWR(
    { url: "/apps", id: "1", body: { start: "1", end: "2" } },
    getAppDailyConversations,
  )
  const { data: getAppDailyEndUsersRes, error: err7 } = useSWR(
    { url: "/apps", id: "1", body: { start: "1", end: "2" } },
    getAppDailyEndUsers,
  )
  const { data: updateAppModelConfigRes, error: err8 } = useSWR(
    { url: "/apps", id: "1", body: { model_id: "gpt-100" } },
    updateAppModelConfig,
  )
  ...
  if (
    !appList ||
    !firstApp ||
    !updateAppSiteStatusRes ||
    !updateAppApiStatusRes ||
    !updateAppRateLimitRes ||
    !updateAppSiteCodeRes ||
    !updateAppSiteConfigRes ||
    !getAppDailyConversationsRes ||
    !getAppDailyEndUsersRes ||
    !updateAppModelConfigRes
  )
    return <Loading />
