...
export const fetchBuiltInToolList = (collectionName: string) => {
  return get<Tool[]>(
    `/workspaces/current/tool-provider/builtin/${collectionName}/tools`,
  )
}

export const fetchCustomToolList = (collectionName: string) => {
  return get<Tool[]>(
    `/workspaces/current/tool-provider/api/tools?provider=${collectionName}`,
  )
}

export const fetchModelToolList = (collectionName: string) => {
  return get<Tool[]>(
    `/workspaces/current/tool-provider/model/tools?provider=${collectionName}`,
  )
}

export const fetchWorkflowToolList = (appID: string) => {
  return get<Tool[]>(
    `/workspaces/current/tool-provider/workflow/tools?workflow_tool_id=${appID}`,
  )
}
...
export const updateBuiltInToolCredential = (
  ...
  return post(
    `/workspaces/current/tool-provider/builtin/${collectionName}/update`,
    {
      body: {
...
export const removeBuiltInToolCredential = (collectionName: string) => {
  return post(
    `/workspaces/current/tool-provider/builtin/${collectionName}/delete`,
    {
      body: {},
    },
  )
...
export const fetchCustomCollection = (collectionName: string) => {
  return get<CustomCollectionBackend>(
    `/workspaces/current/tool-provider/api/get?provider=${collectionName}`,
  )
}
