...
} from "@/components/annotation/type"
...
export const annotationBatchImport: Fetcher<
  { job_id: string; job_status: string },
  { url: string; body: FormData }
> = ({ url, body }) => {
  return post<{ job_id: string; job_status: string }>(
    url,
    { body },
...
export const checkAnnotationBatchImportProgress: Fetcher<
  { job_id: string; job_status: string },
  { jobID: string; appId: string }
> = ({ jobID, appId }) => {
  return get<{ job_id: string; job_status: string }>(
    `/apps/${appId}/annotations/batch-import-status/${jobID}`,
  )
}
...
export const fetchHitHistoryList = (
  appId: string,
  annotationId: string,
  params: Record<string, any>,
) => {
  return get(`apps/${appId}/annotations/${annotationId}/hit-histories`, {
    params,
  })
}
