...
const useConfig = (id: string, payload: CodeNodeType) => {
  ...
  const [allLanguageDefault, setAllLanguageDefault] = useState<Record<
    ...
    CodeNodeType
  > | null>(null)
  const [allLanguageDependencies, setAllLanguageDependencies] = useState<Record<
    CodeLanguage,
    CodeDependency[]
  > | null>(null)
  useEffect(() => {
    if (appId) {
      ;(async () => {
        const { config: javaScriptConfig } = (await fetchNodeDefault(
          appId,
          BlockEnum.Code,
        ...
        const {
          config: pythonConfig,
          available_dependencies: pythonDependencies,
        } = (await fetchNodeDefault(appId, BlockEnum.Code, {
  ...
  const handleChangeDependency = useCallback(
    (index: number, dependency: CodeDependency) => {
      const newInputs = produce(inputs, draft => {
        if (!draft.dependencies) draft.dependencies = []
  ...
  useEffect(() => {
    if (!inputs.code_language) return
    if (!allLanguageDependencies) return
  ...
  useEffect(() => {
    ...
    const newAvailableDependencies = produce(
      allLanguageDependencies[inputs.code_language],
      draft => {
        const currentLanguage = inputs.code_language
        if (!currentLanguage || !draft || !inputs.dependencies) return []
