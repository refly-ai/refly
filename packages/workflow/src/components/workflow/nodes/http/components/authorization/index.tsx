...
const Authorization: FC<Props> = ({ payload, onChange, isShow, onHide }) => {
  const { t } = useTranslation()

  const [tempPayload, setTempPayload] =
    React.useState<AuthorizationPayloadType>(payload)
  const handleAuthTypeChange = useCallback(
    (type: string) => {
      const newPayload = produce(
        tempPayload,
        (draft: AuthorizationPayloadType) => {
              ...
              options={[
                {
                  ...
                  label: t(`${i18nPrefix}.no-auth`),
                },
                {
                  value: AuthorizationType.apiKey,
                  label: t(`${i18nPrefix}.api-key`),
                },
              ]}
            ...
            <>
              ...
              {tempPayload.config?.type === APIType.custom && (
                <Field title={t(`${i18nPrefix}.header`)} isRequired>
                  <input
                    type="text"
                    className="h-8 w-full rounded-lg border-0 bg-gray-100 px-2.5 text-[13px] leading-8 text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-inset focus:ring-gray-200"
