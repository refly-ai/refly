...
import { VarType as VarVarType } from "@/components/workflow/types"
import type { InputVar, ValueSelector, Var } from "@/components/workflow/types"
...
const useConfig = (id: string, payload: ToolNodeType) => {
      ...
      const newInputs = produce(value, draft => {
        ...
        Object.keys(draft.tool_configurations).forEach(key => {
          ...
          if (schema?.type === "boolean") {
            if (typeof value === "string") newConfig[key] = parseInt(value, 10)

            if (typeof value === "boolean") newConfig[key] = value ? 1 : 0
          }
  ...
  useEffect(() => {
    ...
    const inputsWithDefaultValue = produce(inputs, draft => {
      if (
        ...
        draft.tool_configurations = addDefaultValue(
          tool_configurations,
          toolSettingSchema,
        )

      if (!draft.tool_parameters) draft.tool_parameters = {}
    })
  ...
  const filterVar = useCallback(
    (varPayload: Var) => {
      if (currVarType) return varPayload.type === currVarType
  ...
  const handleRun = (submitData: Record<string, any>) => {
    const varTypeInputKeys = Object.keys(inputs.tool_parameters).filter(
      key => inputs.tool_parameters[key].type === VarType.variable,
    )
    ...
    Object.keys(submitData).forEach(key => {
      ...
      varTypeInputKeys.forEach(inputKey => {
        const inputValue = inputs.tool_parameters[inputKey]
          .value as ValueSelector
