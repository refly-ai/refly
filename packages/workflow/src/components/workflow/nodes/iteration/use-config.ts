...
import {
  ...
  isSystemVar,
  toNodeOutputVars,
} from "../_base/components/variable/utils"
...
const useConfig = (id: string, payload: IterationNodeType) => {
  ...
  const handleOutputVarChange = useCallback(
    (
      output: ValueSelector | string,
      _varKindType: VarKindType,
      varInfo?: Var,
    ) => {
      const newInputs = produce(inputs, draft => {
        ...
        draft.output_type =
          ...
          )[outputItemType] || VarType.arrayString
      })
      setInputs(newInputs)
    },
    [inputs, setInputs],
  ...
  const { usedOutVars, allVarObject } = (() => {
    ...
    iterationChildrenNodes.forEach(node => {
      const nodeVars = getNodeUsedVars(node).filter(
      ...
      nodeVars.forEach(varSelector => {
        ...
        if (isInIteration)
          // not pass iteration inner variable
          return
