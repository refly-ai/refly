...
import Editor from "@/components/workflow/nodes/_base/components/prompt/editor"
import AddButton from "@/components/workflow/nodes/_base/components/add-button"
import { DragHandle } from "@/components/base/icons/src/vender/line/others"
...
const ConfigPrompt: FC<Props> = ({
    ...
    (index: number) => {
      return (prompt: string) => {
        const newPrompt = produce(payload as PromptItem[], draft => {
          draft[index][
            draft[index].edition_type === EditionType.jinja2
              ? "jinja2_text"
              : "text"
          ] = prompt
        })
  ...
  const handleCompletionPromptChange = useCallback(
    (prompt: string) => {
      const newPrompt = produce(payload as PromptItem, draft => {
        draft[
          draft.edition_type === EditionType.jinja2 ? "jinja2_text" : "text"
        ] = prompt
      })
      onChange(newPrompt)
    },
      ...
      {isChatModel && Array.isArray(payload) ? (
              ...
              {(payload as PromptItem[]).map((item, index) => {
                ...
                return (
                  <div key={item.id || index} className="group relative">
                    {canDrag && (
                      <DragHandle className="absolute left-[-14px] top-2 hidden h-3.5 w-3.5 text-gray-400 group-hover:block" />
                    )}
                    <ConfigPromptItem
                      ...
                      canRemove={
                        payload.length > 1 &&
                        !(index === 0 && item.role === PromptRole.system)
                      }
                      readOnly={readOnly}
                      id={item.id!}
        ...
        <div>
          <Editor
            ...
            title={
              <span className="capitalize">{t(`${i18nPrefix}.prompt`)}</span>
            }
            value={
