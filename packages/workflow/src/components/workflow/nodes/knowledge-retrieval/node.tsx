    ...
    ;(async () => {
      ...
      if (data.dataset_ids?.length > 0) {
        const { data: dataSetsWithDetail } = await fetchDatasets({
          url: "/datasets",
          params: { page: 1, ids: data.dataset_ids },
        })
        //  avoid old data overwrite new data
        if (currUpdateTime < updateTime.current) return
    ...
    <div className="mb-1 px-3 py-1">
      <div className="space-y-0.5">
        {selectedDatasets.map(({ id, name }) => (
          <div
            key={id}
            className="flex h-[26px] items-center rounded-md bg-gray-100 px-1 text-xs font-normal text-gray-700">
            <div className="mr-1 shrink-0 rounded-md border-[0.5px] border-[#E0EAFF] bg-[#F5F8FF] p-1">
              <Folder className="h-3 w-3 text-[#444CE7]" />
            </div>
            <div className="w-0 grow truncate text-xs font-normal text-gray-700">
              {name}
            </div>
          </div>
        ))}
