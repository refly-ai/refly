...
const useConfig = (id: string, payload: KnowledgeRetrievalNodeType) => {
  ...
  const { inputs, setInputs: doSetInputs } =
    useNodeCrud<KnowledgeRetrievalNodeType>(id, payload)
  ...
  const handleCompletionParamsChange = useCallback(
    (newParams: Record<string, any>) => {
      ...
      if (
        isEqual(
          newParams,
          inputRef.current.single_retrieval_config?.model.completion_params,
        )
      )
        return
  ...
  useEffect(() => {
    ...
    if (
      inputs.retrieval_mode === RETRIEVE_TYPE.multiWay &&
      inputs.multiple_retrieval_config?.reranking_model?.provider
    )
      return

    if (
      inputs.retrieval_mode === RETRIEVE_TYPE.oneWay &&
      inputs.single_retrieval_config?.model?.provider
    )
      return
  ...
  const handleRetrievalModeChange = useCallback(
    (newMode: RETRIEVE_TYPE) => {
      const newInputs = produce(inputs, draft => {
        draft.retrieval_mode = newMode
        if (newMode === RETRIEVE_TYPE.multiWay) {
          draft.multiple_retrieval_config = {
            top_k:
              draft.multiple_retrieval_config?.top_k || DATASET_DEFAULT.top_k,
            score_threshold: draft.multiple_retrieval_config?.score_threshold,
            reranking_model: !draft.multiple_retrieval_config?.reranking_model
              ?.provider
              ? {
                  provider: rerankDefaultModel?.provider?.provider || "",
                  model: rerankDefaultModel?.model || "",
  ...
  const handleMultipleRetrievalConfigChange = useCallback(
    (newConfig: MultipleRetrievalConfig) => {
      const newInputs = produce(inputs, draft => {
  ...
  useEffect(() => {
    ...
    if (
      isChatMode &&
      inputs.query_variable_selector.length === 0 &&
      startNodeId
    )
