...
import KnowledgeRetrievalDefault from "@/components/workflow/nodes/knowledge-retrieval/default"
...
import TemplateTransformDefault from "@/components/workflow/nodes/template-transform/default"
import QuestionClassifyDefault from "@/components/workflow/nodes/question-classifier/default"
import HTTPDefault from "@/components/workflow/nodes/http/default"
import ToolDefault from "@/components/workflow/nodes/tool/default"
import VariableAssigner from "@/components/workflow/nodes/variable-assigner/default"
import ParameterExtractorDefault from "@/components/workflow/nodes/parameter-extractor/default"
import IterationDefault from "@/components/workflow/nodes/iteration/default"
import { ssePost } from "@/service/base"

import { getInputVars as doGetInputVars } from "@/components/base/prompt-editor/constants"
...
const varTypeToInputVarType = (
  ...
  if (type === VarType.arrayFile) return InputVarType.files
...
const useOneStepRun = <T>({
  ...
  const availableNodesIncludeParent =
  ...
  const getVar = (valueSelector: ValueSelector): Var | undefined => {
    ...
    const targetVar = isSystem
      ? allOutputVars.find(item => !!item.isStartNode)
      : allOutputVars.find(v => v.nodeId === valueSelector[0])
    if (!targetVar) return undefined
    if (isSystem)
      return targetVar.vars.find(
        item => item.variable.split(".")[1] === valueSelector[1],
      )
  ...
  const [runInputData, setRunInputData] = useState<Record<string, any>>(
    defaultRunInputData || {},
  )
  const iterationTimes = iteratorInputKey
    ? runInputData[iteratorInputKey].length
    : 0
  const [runResult, setRunResult] = useState<any>(null)

  const {
    handleNodeDataUpdate,
  }: { handleNodeDataUpdate: (data: any) => void } = useNodeDataUpdate()
  ...
  useEffect(() => {
    ...
    if (data._isSingleRun) {
      const { isValid, errorMessage } = checkValid(
        data,
        t,
        moreDataForCheckValid,
      )
        ...
        ssePost(
          ...
          { body: { inputs: submitData } },
          {
            ...
            onIterationNext: () => {
              ...
              if (_iterationResult.length >= iterationTimes!) return
            ...
            onNodeFinished: params => {
              ...
              const { data } = params
              const currentIndex = iterationRunResult[
                iterationRunResult.length - 1
              ].findIndex(trace => trace.node_id === data.node_id)
              const newIterationRunResult = produce(
                iterationRunResult,
                draft => {
                  if (currentIndex > -1) {
  ...
  const getInputVars = (textList: string[]) => {
    ...
    const variables = unionBy(valueSelectors, item => item.join(".")).map(
      item => {
        const varInfo = getNodeInfoById(
          availableNodesIncludeParent,
          item[0],
        )?.data
        ...
        return {
          label: {
            ...
            nodeName:
              varInfo?.title || availableNodesIncludeParent[0]?.data.title, // default start node title
            variable: isSystemVar(item)
              ? item.join(".")
              : item[item.length - 1],
          },
          variable: `#${item.join(".")}#`,
