...
const CodeEditor: FC<Props> = ({
  ...
  const main = (
    <>
      ...
      />
      {!outPutValue && (
        <div className="pointer-events-none absolute left-[36px] top-0 text-[13px] font-normal leading-[18px] text-gray-300">
          {placeholder}
        </div>
      )}
    </>
      ...
      {noWrapper ? (
        <div
          ...
          style={{
            height: isExpand
              ? "100%"
              : editorContentHeight / 2 + CODE_EDITOR_LINE_HEIGHT, // In IDE, the last line can always be in lop line. So there is some blank space in the bottom.
