  ...
  return (
    <PortalToFollowElem
      ...
      <PortalToFollowElemTrigger onClick={() => handleOpenChange(!open)}>
        <div
          className={`flex h-6 w-6 cursor-pointer items-center justify-center rounded-md hover:bg-black/5 ${open && "bg-black/5"} ${triggerClassName} `}>
          <RiMoreFill
            className={`h-4 w-4 ${inNode ? "text-gray-500" : "text-gray-700"}`}
          />
        </div>
      </PortalToFollowElemTrigger>
