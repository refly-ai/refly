...
const AddVariablePopupWithPosition = ({
  ...
  const outputType = useMemo(() => {
    if (!showAssignVariablePopup) return ""

    const groupEnabled =
      showAssignVariablePopup.variableAssignerNodeData.advanced_settings
        ?.group_enabled
    ...
    const group =
      showAssignVariablePopup.variableAssignerNodeData.advanced_settings?.groups.find(
        group =>
          group.groupId ===
  ...
  const handleAddVariable = useCallback(
    (value: ValueSelector, varDetail: Var) => {
  ...
  if (!showAssignVariablePopup) return null
