...
import { useNodesReadOnly, useWorkflow } from "@/components/workflow/hooks"

const useConfig = (id: string, payload: VariableAssignerNodeType) => {
  const { nodesReadOnly: readOnly } = useNodesReadOnly()
  const { handleOutVarRenameChange, isVarUsedInNodes, removeUsedVarInNodes } =
    useWorkflow()

  const { inputs, setInputs } = useNodeCrud<VariableAssignerNodeType>(
    id,
    payload,
  )
  ...
  const handleListOrTypeChangeInGroup = useCallback(
    (groupId: string) => {
      return (payload: VarGroupItem) => {
        const index = inputs.advanced_settings.groups.findIndex(
  ...
  const [removeType, setRemoveType] = useState<"group" | "enableChanged">(
    "group",
  )
    ...
    (groupId: string) => {
      return () => {
        ...
        if (
          isVarUsedInNodes([
            id,
            inputs.advanced_settings.groups[index].group_name,
            "output",
          ])
        ) {
          showRemoveVarConfirm()
          setRemovedVars([
            [id, inputs.advanced_settings.groups[index].group_name, "output"],
          ])
          setRemoveType("group")
        ...
        if (enabled) {
          if (draft.advanced_settings.groups.length === 0) {
            ...
            handleOutVarRenameChange(
              id,
              [id, "output"],
              [id, DEFAULT_GROUP_NAME, "output"],
            )
          }
        } else {
          if (draft.advanced_settings.groups.length > 0) {
            if (draft.advanced_settings.groups.length > 1) {
              const useVars = draft.advanced_settings.groups.filter(
                (item, index) =>
                  index > 0 &&
                  isVarUsedInNodes([id, item.group_name, "output"]),
              )
              if (useVars.length > 0) {
                showRemoveVarConfirm()
                setRemovedVars(
                  useVars.map(item => [id, item.group_name, "output"]),
                )
            ...
            handleOutVarRenameChange(
              id,
              [id, draft.advanced_settings.groups[0].group_name, "output"],
              [id, "output"],
            )
          }
        }
