...
import { useIsChatMode, useNodeDataUpdate, useWorkflow } from "../../hooks"
import type { Node, ValueSelector, Var } from "../../types"
import { useWorkflowStore } from "../../store"
import type { VarGroupItem, VariableAssignerNodeType } from "./types"
import { toNodeAvailableVars } from "@/components/workflow/nodes/_base/components/variable/utils"
...
export const useVariableAssigner = () => {
  ...
  const handleAssignVariableValueChange = useCallback(
    (
      nodeId: string,
      value: ValueSelector,
      varDetail: Var,
      ...
      const node: Node<VariableAssignerNodeType> = nodes.find(
        node => node.id === nodeId,
      )!
      ...
      if (groupId && groupId !== "target") {
        payload = {
          advanced_settings: {
            ...
            groups: node.data.advanced_settings?.groups.map(
              (group: VarGroupItem & { groupId: string }) => {
                if (
                  group.groupId === groupId &&
                  !group.variables.some(
                    item =>
                      item.join(".") === (value as ValueSelector).join("."),
                  )
                ) {
                  return {
                    ...group,
        ...
        if (
          node.data.variables.some(
            item => item.join(".") === (value as ValueSelector).join("."),
          )
        )
          return
        payload = {
  ...
  const handleAddVariableInAddVariablePopupWithPosition = useCallback(
    (
      ...
      handleAssignVariableValueChange(
        ...
        value,
        varDetail,
        variableAssignerNodeHandleId,
      )
    },
    [store, workflowStore, handleAssignVariableValueChange],
  )

  const handleGroupItemMouseEnter = useCallback(
    (groupId: string) => {
  ...
  const handleGroupItemMouseLeave = useCallback(() => {
    const { connectingNodePayload, setHoveringAssignVariableGroupId } =
      workflowStore.getState()

    if (connectingNodePayload) setHoveringAssignVariableGroupId(undefined)
  }, [workflowStore])
...
export const useGetAvailableVars = () => {
  ...
  const getAvailableVars = useCallback(
    (nodeId: string, handleId: string, filterVar: (v: Var) => boolean) => {
      ...
      return toNodeAvailableVars({
        parentNode,
        t,
        beforeNodes: uniqBy(availableNodes, "id").filter(
          node => node.id !== nodeId,
        ),
        isChatMode,
        filterVar,
