...
import type { ToolParameter } from "@/components/tools/types"
...
function toParmExactParams(toolParams: ToolParameter[], lan: string): Param[] {
  return toolParams.map(item => {
    return {
      ...
      options:
        item.options?.map(option => option.label[lan] || option.label.en_US) ||
        [],
    }
  })
}
const ImportFromTool: FC<Props> = ({ onImport }) => {
  ...
  const handleSelectTool = useCallback(
    (_type: BlockEnum, toolInfo?: ToolDefaultValue) => {
      ...
      const toExactParams = (currTool?.parameters || []).filter(
