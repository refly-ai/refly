...
import { checkHasQueryBlock } from "@/components/base/prompt-editor/constants"
...
const useConfig = (id: string, payload: ParameterExtractorNodeType) => {
  ...
  const { inputs, setInputs: doSetInputs } =
    useNodeCrud<ParameterExtractorNodeType>(id, payload)
  ...
  const handleExactParamsChange = useCallback(
    (newParams: Param[], moreInfo?: MoreInfo) => {
      ...
      if (
        moreInfo &&
        moreInfo?.type === ChangeType.changeVarName &&
        moreInfo.payload
      )
  ...
  const appendDefaultPromptConfig = useCallback(
    (
      draft: ParameterExtractorNodeType,
      defaultConfig: any,
      _passInIsChatMode?: boolean,
    ) => {
  ...
  const { currentModel: currModel } =
    useTextGenerationCurrentProviderAndModelAndModelList({
      provider: model.provider,
      model: model.name,
    })

  const isSupportFunctionCall =
    currModel?.features?.includes(ModelFeatureEnum.toolCall) ||
    currModel?.features?.includes(ModelFeatureEnum.multiToolCall)
