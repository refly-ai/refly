...
const RunPanel: FC<RunProps> = ({
  ...
  const getResult = useCallback(
    async (appID: string, runID: string) => {
      try {
      ...
      if (node_type !== BlockEnum.Iteration) {
        ...
        iterationIndexInfos.forEach(({ start, end }) => {
          if (index >= start && index < end) {
            if (index === start) isIterationFirstNode = true
        ...
        if (isInIteration) {
          ...
          if (isIterationFirstNode) iterationDetails!.push([item])
          else iterationDetails[iterationDetails.length - 1].push(item)
  ...
  const adjustResultHeight = () => {
    if (ref.current) setHieght(ref.current?.clientHeight - 16 - 16 - 2 - 1)
  }
