...
const IterationResultPanel: FC<Props> = ({ list, onHide, onBack, noWrap }) => {
  ...
  const main = (
    <>
      <div className={cn(!noWrap && "shrink-0", "pl-4 pr-3 pt-3")}>
        ...
        <div
          className="text-primary-600 flex cursor-pointer items-center space-x-1 py-2"
          onClick={onBack}>
      ...
      <div
        className={cn(
          !noWrap ? "h-0 grow" : "max-h-full",
          "overflow-y-auto bg-gray-50 px-4 pb-4",
        )}>
        {list.map((iteration, index) => (
          <div key={index} className={cn("my-4", index === 0 && "mt-2")}>
            <div className="flex items-center">
              <div className="shrink-0 text-xs font-semibold uppercase leading-[18px] text-gray-500">
  ...
  return (
    <div
      ...
      <div className="flex h-full flex-col rounded-2xl bg-white">{main}</div>
    </div>
  )
}
