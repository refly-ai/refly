...
import { fetchPublishedWorkflow, stopWorkflowRun } from "@/service/workflow"
...
export const useWorkflowRun = () => {
  ...
  const handleLoadBackupDraft = useCallback(() => {
    ...
    if (backupDraft) {
      const { nodes, edges, viewport, features } = backupDraft
      ...
      ssePost(
        ...
        },
        {
          ...
          onNodeStarted: params => {
            ...
            const { workflowRunningData, setWorkflowRunningData } =
              workflowStore.getState()
            const { getNodes, setNodes, edges, setEdges, transform } =
              store.getState()
            if (isInIteration) {
              setWorkflowRunningData(
                produce(workflowRunningData!, draft => {
                  ...
                  const currIteration =
                    iterations.details![iterations.details!.length - 1]
                  currIteration.push({
                    ...data,
              ...
              if (!currentNode.parentId) {
                setViewport({
                  x:
                    (clientWidth - 400 - currentNode.width! * zoom) / 2 -
                    position.x * zoom,
                  y:
                    (clientHeight - currentNode.height! * zoom) / 2 -
                    position.y * zoom,
                  zoom: transform[2],
                })
              }
              const newNodes = produce(nodes, draft => {
                draft[currentNodeIndex].data._runningStatus =
                  NodeRunningStatus.Running
              })
              setNodes(newNodes)
              const newEdges = produce(edges, draft => {
                const edge = draft.find(
                  edge =>
                    edge.target === data.node_id && edge.source === prevNodeId,
                )
          ...
          onNodeFinished: params => {
            ...
            if (isInIteration) {
              ...
              setWorkflowRunningData(
                produce(workflowRunningData!, draft => {
                  const currentIndex = draft.tracing!.findIndex(
                    trace => trace.node_id === data.node_id,
                  )
          ...
          onIterationStart: params => {
            ...
            const newNodes = produce(nodes, draft => {
              draft[currentNodeIndex].data._runningStatus =
                NodeRunningStatus.Running
              draft[currentNodeIndex].data._iterationLength =
                data.metadata.iterator_length
            })
            setNodes(newNodes)
            const newEdges = produce(edges, draft => {
              const edge = draft.find(
          ...
          onIterationNext: params => {
            ...
            setWorkflowRunningData(
              produce(workflowRunningData!, draft => {
                ...
                if (iteration.details!.length >= iterationLength) return
