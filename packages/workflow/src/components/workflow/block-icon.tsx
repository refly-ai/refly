...
import {
  ...
  TemplatingTransform,
  VariableX,
} from "@/components/base/icons/src/vender/workflow"
...
const getIcon = (type: BlockEnum, className: string) => {
  return {
    ...
    [BlockEnum.ParameterExtractor]: (
      <ParameterExtractor className={className} />
    ),
  }[type]
}
...
const BlockIcon: FC<BlockIconProps> = ({
  ...
  return (
    <div
      className={`border-white/2 flex items-center justify-center border-[0.5px] text-white ${ICON_CONTAINER_CLASSNAME_SIZE_MAP[size]} ${ICON_CONTAINER_BG_COLOR_MAP[type]} ${toolIcon && "!shadow-none"} ${className} `}>
      {type !== BlockEnum.Tool &&
