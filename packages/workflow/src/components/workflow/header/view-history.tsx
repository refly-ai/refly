...
import {
  ClockPlay,
  ClockPlaySlim,
} from "@/components/base/icons/src/vender/line/time"
...
import { useStore, useWorkflowStore } from "@/components/workflow/store"
...
const ViewHistory = ({ withText }: ViewHistoryProps) => {
  ...
  const { data: runList, isLoading: runListLoading } = useSWR(
    appDetail && !isChatMode && open
      ? `/apps/${appDetail.id}/workflow-runs`
      : null,
    fetchWorkflowRunHistory,
  )
  const { data: chatList, isLoading: chatListLoading } = useSWR(
    appDetail && isChatMode && open
      ...
      <PortalToFollowElemTrigger onClick={() => setOpen(v => !v)}>
        {withText && (
          <div
            ...
            <ClockPlay className={"mr-1 h-4 w-4"} />
            {t("workflow.common.showRunHistory")}
          </div>
        )}
        {!withText && (
          <TooltipPlus popupContent={t("workflow.common.viewRunHistory")}>
            <div
              className={`flex h-7 w-7 cursor-pointer items-center justify-center rounded-md hover:bg-black/5 ${open && "bg-primary-50"} `}
              onClick={() => {
                setCurrentLogItem()
                setShowMessageLogModal(false)
              }}>
              <ClockPlay
                className={`h-4 w-4 ${open ? "text-primary-600" : "text-gray-500"}`}
              />
            </div>
          </TooltipPlus>
        )}
