...
import { ChecklistSquare } from "@/components/base/icons/src/vender/line/general"
  ...
  return (
    <PortalToFollowElem
      ...
      <PortalToFollowElemTrigger onClick={() => !disabled && setOpen(v => !v)}>
        <div
          className={cn(
            "shadow-xs relative flex h-8 w-8 items-center justify-center rounded-lg border-[0.5px] border-gray-200 bg-white p-0.5",
            disabled && "cursor-not-allowed opacity-50",
          )}>
          <div
            className={`hover:bg-primary-50 group flex h-full w-full cursor-pointer items-center justify-center rounded-md ${open && "bg-primary-50"} `}>
            <RiListCheck3
              className={`group-hover:text-primary-600 h-4 w-4 ${open ? "text-primary-600" : "text-gray-500"}`}
            />
          </div>
