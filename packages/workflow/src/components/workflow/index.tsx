...
const Workflow: FC<WorkflowProps> = memo(
  ({ nodes: originalNodes, edges: originalEdges, viewport }) => {
    ...
    useKeyPress(
      `${getKeyboardKeyCodeBySystem("ctrl")}.v`,
      e => {
        if (isEventTargetInputArea(e.target as HTMLElement)) return
    ...
    useKeyPress(
      [
        `${getKeyboardKeyCodeBySystem("ctrl")}.y`,
    ...
    return (
      <div
        ...
        className={`relative h-full w-full min-w-[960px] bg-[#F0F2F7] ${workflowReadOnly && "workflow-panel-animation"} ${nodeAnimation && "workflow-node-animation"} `}
...
const WorkflowWrap = memo(() => {
  ...
  if (!data || isLoading) {
    return (
      <div className="relative flex h-full w-full items-center justify-center bg-[#F0F2F7]">
        <Loading />
      </div>
    )
  }
