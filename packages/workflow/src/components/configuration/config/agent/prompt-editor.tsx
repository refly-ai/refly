...
const Editor: FC<Props> = ({ className, type, value, onChange }) => {
    ...
    setShowExternalDataToolModal({
      ...
      onValidateBeforeSaveCallback: (newExternalDataTool: ExternalDataTool) => {
        ...
        for (let i = 0; i < externalDataToolsConfig.length; i++) {
          if (
            externalDataToolsConfig[i].variable === newExternalDataTool.variable
          ) {
            notify({
              type: "error",
              message: t("appDebug.varKeyError.keyAlreadyExists", {
                key: externalDataToolsConfig[i].variable,
              }),
            })
            return false
          }
      ...
      <div className="rounded-xl bg-white">
        <div
          ...
          <div className="text-sm font-semibold uppercase text-indigo-800">
            {t(
              `appDebug.agent.${isFirstPrompt ? "firstPrompt" : "nextIteration"}`,
            )}
          </div>
        ...
        <div
          className={cn(
            editorHeight,
            "min-h-[102px] overflow-y-auto px-4 text-sm text-gray-700",
          )}>
          <PromptEditor
