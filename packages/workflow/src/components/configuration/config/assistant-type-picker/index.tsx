...
import { BubbleText } from "@/components/base/icons/src/vender/solid/education"
import Radio from "@/components/base/radio/ui"
  ...
  return (
    <div
      className={cn(
        ...
        isChecked
          ? "border-[2px] border-indigo-600 shadow-sm"
          : "border border-gray-100",
        "bg-gray-25 mb-2 rounded-xl p-3 pr-4 hover:bg-gray-50",
      )}
      ...
      <div className="ml-9 text-xs font-normal leading-[18px] text-gray-500">
        {description}
      </div>
    </div>
  )
...
const AssistantTypePicker: FC<Props> = ({
  ...
  const agentConfigUI = (
    <>
      ...
      <div
        className={cn(
          isAgent ? "hover:bg-primary-50 group cursor-pointer" : "opacity-30",
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <div className="text-sm font-medium leading-5 text-gray-900 group-hover:text-[#155EEF]">
        ...
        <div className="ml-9 text-xs font-normal leading-[18px] text-gray-500">
          {t("appDebug.agent.setting.description")}
        </div>
      </div>
    </>
  )
      ...
      <PortalToFollowElem
        ...
        <PortalToFollowElemTrigger onClick={() => setOpen(v => !v)}>
          <div
            className={cn(
              open && "bg-gray-50",
              "flex h-8 cursor-pointer select-none items-center space-x-1 rounded-lg border border-black/5 px-3 text-indigo-600",
            )}>
            {isAgent ? (
              <BubbleText className="h-3 w-3" />
            ) : (
              <CuteRobote className="h-3 w-3" />
            )}
            <div className="text-xs font-medium">
              {t(
                `appDebug.assistantType.${isAgent ? "agentAssistant" : "chatAssistant"}.name`,
              )}
            </div>
        ...
        <PortalToFollowElemContent style={{ zIndex: 1000 }}>
          <div className="border-black/8 relative left-0.5 w-[480px] rounded-xl border bg-white p-6 shadow-lg">
            <div className="mb-2 text-sm font-semibold leading-5 text-gray-900">
            ...
            <SelectItem
              ...
              description={t(
                "appDebug.assistantType.chatAssistant.description",
              )}
              isChecked={!isAgent}
            ...
            <SelectItem
              ...
              description={t(
                "appDebug.assistantType.agentAssistant.description",
              )}
              isChecked={isAgent}
              onClick={handleChange}
            />
