...
import {
  ...
  type MoreLikeThisConfig,
  type PromptVariable,
  type SpeechToTextConfig,
  type SuggestedQuestionsAfterAnswerConfig,
  type TextToSpeechConfig,
} from "@/models/debug"
import type { AppType } from "@/types/app"
import { ModelModeType } from "@/types/app"
import { useModalContext } from "@/context/modal-context"
import ConfigParamModal from "@/components/configuration/toolbox/annotation/config-param-modal"
...
const Config: FC = () => {
  ...
  const { featureConfig, handleFeatureChange } = useFeature({
    ...
    setMoreLikeThis: value => {
      setMoreLikeThisConfig(
        produce(moreLikeThisConfig, (draft: MoreLikeThisConfig) => {
          draft.enabled = value
        }),
      )
    },
    ...
    setSpeechToText: value => {
      setSpeechToTextConfig(
        produce(speechToTextConfig, (draft: SpeechToTextConfig) => {
          draft.enabled = value
        }),
      )
    },
    ...
    setTextToSpeech: value => {
      setTextToSpeechConfig(
        produce(textToSpeechConfig, (draft: TextToSpeechConfig) => {
          draft.enabled = value
    ...
    setCitation: value => {
      setCitationConfig(
        produce(citationConfig, (draft: CitationConfig) => {
          draft.enabled = value
        }),
      )
      formattingChangedDispatcher()
    ...
    setModeration: value => {
      setModerationConfig(
        produce(moderationConfig, (draft: ModerationConfig) => {
          draft.enabled = value
        }),
      )
  ...
  const hasChatConfig =
    ...
    (featureConfig.openingStatement ||
      ...
      (featureConfig.textToSpeech && !!text2speechDefaultModel) ||
      featureConfig.citation)
  const hasCompletionConfig =
    !isChatApp &&
