...
import { ModelFeatureEnum } from "@/components/header/account-setting/model-provider-page/declarations"
...
import SelectDataSet from "@/components/configuration/dataset-config/select-dataset"
...
import type { FormValue } from "@/components/header/account-setting/model-provider-page/declarations"
import { useTextGenerationCurrentProviderAndModelAndModelList } from "@/components/header/account-setting/model-provider-page/hooks"
import { fetchCollectionList } from "@/service/tools"
import { type Collection } from "@/components/tools/types"
...
const Configuration: FC = () => {
  ...
  const [publishedConfig, setPublishedConfig] = useState<PublishConfig | null>(
    null,
  )

  const modalConfig = useMemo(
    () => appDetail?.model_config || ({} as BackendModelConfig),
    [appDetail],
  )
  ...
  const [
    isShowDebugPanel,
    { setTrue: showDebugPanel, setFalse: hideDebugPanel },
  ] = useBoolean(false)
  ...
  const [annotationConfig, doSetAnnotationConfig] =
    useState<AnnotationReplyConfig>({
      id: "",
      enabled: false,
  ...
  const setAnnotationConfig = (
    config: AnnotationReplyConfig,
    notSetFormatChanged?: boolean,
  ) => {
  ...
  const [
    isShowHistoryModal,
    { setTrue: showHistoryModal, setFalse: hideHistoryModal },
  ] = useBoolean(false)
  ...
  const syncToPublishedConfig = (_publishedConfig: PublishConfig) => {
    ...
    setCitationConfig(
      modelConfig.retriever_resource || {
        enabled: false,
      },
    )
  }
  ...
  const { currentModel: currModel, textGenerationModelList } =
  ...
  const isFunctionCall = (() => {
    ...
    return (
      features.includes(ModelFeatureEnum.toolCall) ||
      features.includes(ModelFeatureEnum.multiToolCall)
    )
  })()
  ...
  const handleSetVisionConfig = (
    config: VisionSettings,
    notNoticeFormattingChanged?: boolean,
  ) => {
    doSetVisionConfig({
  ...
  useEffect(() => {
    ;(async () => {
      ...
      fetchAppDetail({ url: "/apps", id: appId }).then(async (res: any) => {
        ...
        // old dataset struct
        if (
          modelConfig.agent_mode?.tools?.find(
            ({ dataset }: any) => dataset?.enabled,
          )
        )
          datasets = modelConfig.agent_mode?.tools.filter(
            ({ dataset }: any) => dataset?.enabled,
          )
        // new dataset struct
                ...
                ? {
                    ...
                    tools: modelConfig.agent_mode?.tools
                      ...
                      .map((tool: any) => {
                        return {
                          ...tool,
                          isDeleted: res.deleted_tools?.includes(
                            tool.tool_name,
                          ),
                          notAuthor:
                            collectionList.find(c => tool.provider_id === c.id)
  ...
  const promptEmpty = (() => {
    if (mode !== AppType.completion) return false

    if (isAdvancedMode) {
      if (modelModeType === ModelModeType.chat)
        return chatPromptConfig.prompt.every(({ text }: any) => !text)
      else return !completionPromptConfig.prompt?.text
    } else {
  ...
  const cannotPublish = (() => {
    if (mode !== AppType.completion) {
      ...
      if (modelModeType === ModelModeType.completion) {
        if (!hasSetBlockStatus.history || !hasSetBlockStatus.query) return true
  ...
  const contextVarEmpty =
    mode === AppType.completion && dataSets.length > 0 && !hasSetContextVar
  const onPublish = async (modelAndParameter?: ModelAndParameter) => {
    ...
    if (promptEmpty) {
      notify({
        type: "error",
        message: t("appDebug.otherError.promptNoBeEmpty"),
        duration: 3000,
      })
      return
    }
    if (isAdvancedMode && mode !== AppType.completion) {
      if (modelModeType === ModelModeType.completion) {
        if (!hasSetBlockStatus.history) {
          notify({
            type: "error",
            message: t("appDebug.otherError.historyNoBeEmpty"),
            duration: 3000,
          })
          return
        }
        if (!hasSetBlockStatus.query) {
          notify({
            type: "error",
            message: t("appDebug.otherError.queryNoBeEmpty"),
            duration: 3000,
          })
          return
        }
    ...
    if (contextVarEmpty) {
      notify({
        type: "error",
        message: t("appDebug.feature.dataSet.queryVariable.contextVarNotEmpty"),
        duration: 3000,
      })
      return
    }
    ...
    const data: BackendModelConfig = {
      ...
      model: {
        ...
        completion_params:
          modelAndParameter?.parameters || (completionParams as any),
      },
      dataset_configs: {
        ...datasetConfigs,
        datasets: {
    ...
    const newModelConfig = produce(modelConfig, (draft: any) => {
      ...
      draft.suggested_questions_after_answer =
        suggestedQuestionsAfterAnswerConfig
      ...
      <>
        <div className="flex h-full flex-col">
          <div className="relative flex h-[200px] grow pt-14">
            {/* Header */}
            <div className="absolute left-0 top-0 h-14 w-full bg-white">
              <div className="flex h-14 items-center justify-between px-6">
                <div className="flex items-center">
                  ...
                  <div className="flex h-[14px] items-center space-x-1 text-xs">
                    {isAdvancedMode && (
                      <div className="ml-1 flex h-5 items-center rounded-md border border-gray-100 px-1.5 text-[11px] font-medium uppercase text-gray-500">
                ...
                <div className="flex items-center">
                  ...
                  {!debugWithMultipleModel && (
                    <>
                      ...
                      />
                      <div className="mx-2 h-[14px] w-[1px] bg-gray-200"></div>
                    </>
                  )}
                  {isMobile && (
                    <Button
                      ...
                      <span className="mr-1">
                        {t("appDebug.operation.debugConfig")}
                      </span>
            ...
            <div
              className={`flex h-full w-full shrink-0 flex-col sm:w-1/2 ${debugWithMultipleModel && "max-w-[560px]"}`}>
              <Config />
            </div>
            {!isMobile && (
              <div
                ...
                <div className="flex h-0 grow flex-col rounded-tl-2xl border-l border-t bg-gray-50">
                  <Debug
