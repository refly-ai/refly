...
import AdvancedMessageInput from "@/components/configuration/config-prompt/advanced-prompt-input"
...
const Prompt: FC<IPromptProps> = ({
  ...
  const handleValueChange = (value: string, index?: number) => {
    if (modelModeType === ModelModeType.chat) {
      const newPrompt = produce(
        currentAdvancedPrompt as PromptItem[],
        draft => {
  ...
  const handleAddMessage = () => {
    ...
    const lastMessageType =
      currentAdvancedPromptList[currentAdvancedPromptList.length - 1]?.role
    const appendMessage = {
      role:
  ...
  return (
    <div>
      ...
      {modelModeType === ModelModeType.chat &&
        (currentAdvancedPrompt as PromptItem[]).length <
