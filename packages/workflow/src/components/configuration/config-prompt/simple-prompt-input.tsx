...
import AutomaticBtn from "@/components/configuration/config/automatic/automatic-btn"
import type { AutomaticRes } from "@/service/debug"
import GetAutomaticResModal from "@/components/configuration/config/automatic/get-automatic-res"
...
import { ADD_EXTERNAL_DATA_TOOL } from "@/components/configuration/config-var"
import { INSERT_VARIABLE_VALUE_BLOCK_COMMAND } from "@/components/base/prompt-editor/plugins/variable-block"
import { PROMPT_EDITOR_UPDATE_VALUE_BY_EVENT_EMITTER } from "@/components/base/prompt-editor/plugins/update-block"
...
const Prompt: FC<ISimplePromptInput> = ({
  ...
  const handleAutomaticRes = (res: AutomaticRes) => {
    ...
    if (mode !== AppType.completion) setIntroduction(res.opening_statement)
    showAutomaticFalse()
    eventEmitter?.emit({
  ...
  return (
    <div
      className={cn(
        !readonly ? `${s.gradientBorder}` : "bg-gray-50",
        "relative shadow-md",
      )}>
          ...
          <div className="flex items-center space-x-1">
            <div className="h2">
              {mode !== AppType.completion
                ? t("appDebug.chatSubTitle")
                : t("appDebug.completionSubTitle")}
            </div>
            {!readonly && (
              <Tooltip
                htmlContent={
                  <div className="w-[180px]">{t("appDebug.promptTip")}</div>
                }
                selector="config-prompt-tooltip">
                <RiQuestionLine className="h-[14px] w-[14px] text-indigo-400" />
              </Tooltip>
            )}
          </div>
