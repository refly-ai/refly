...
import type { IInputTypeIconProps } from "@/components/configuration/config-var/input-type-icon"
import IconTypeIcon from "@/components/configuration/config-var/input-type-icon"
...
const VarItem: FC<{ item: Option }> = ({ item }) => (
  <div className="flex h-[18px] items-center space-x-1 rounded bg-[#EFF8FF] px-1">
    <IconTypeIcon
      type={item.type as IInputTypeIconProps["type"]}
  ...
  return (
    <PortalToFollowElem
        ...
        <div
          className={cn(
            ...
            notSetVar
              ...
              : "text-primary-600 border-gray-200 hover:bg-gray-50",
            open ? "bg-gray-50" : "bg-white",
            `shadow-xs flex h-8 cursor-pointer items-center justify-center space-x-1 rounded-lg border px-2 text-[13px] font-medium`,
          )}>
          <div>
            {value ? (
      ...
      <PortalToFollowElemContent style={{ zIndex: 1000 }}>
        {options.length > 0 ? (
          <div className="max-h-[50vh] w-[240px] overflow-y-auto rounded-lg border border-gray-200 bg-white p-1 shadow-lg">
            {options.map(({ name, value, type }, index) => (
              <div
                key={index}
                className="flex cursor-pointer rounded-lg px-3 py-1 hover:bg-gray-50"
                onClick={() => {
                  onChange(value)
                  setOpen(false)
                }}>
          ...
          <div className="w-[240px] rounded-lg border border-gray-200 bg-white p-6 shadow-lg">
            <div className="mb-1 text-sm font-medium text-gray-700">
              {t("appDebug.feature.dataSet.queryVariable.noVar")}
            </div>
            <div className="text-xs leading-normal text-gray-500">
