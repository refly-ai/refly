...
const CardItem: FC<ICardItemProps> = ({
  ...
  return (
    <div
      ...
      <div className="flex items-center space-x-2">
        ...
        <div>
          <div className="mr-1 flex w-[160px] items-center">
            <div
              className={cn(
                "overflow-hidden text-ellipsis whitespace-nowrap text-[13px] font-medium leading-[18px] text-gray-800",
                !config.embedding_available && "opacity-50",
              )}>
              {config.name}
            </div>
            {!config.embedding_available && (
              <Tooltip
                ...
                <span className="boder-gray-200 inline-flex shrink-0 whitespace-nowrap rounded-md border px-1 text-xs font-normal leading-[18px] text-gray-500">
          ...
          <div
            className={cn(
              "flex max-w-[150px] text-xs text-gray-500",
              !config.embedding_available && "opacity-50",
            )}>
      ...
      {!readonly && (
        <RemoveIcon
          className={`${s.deleteBtn} absolute right-1 top-1/2 translate-y-[-50%]`}
