...
import { Tool03 } from "@/components/base/icons/src/vender/solid/general"
...
const Tools = () => {
  ...
  const { externalDataToolsConfig, setExternalDataToolsConfig, modelConfig } =
    useContext(ConfigContext)
  const [expanded, setExpanded] = useState(true)
  const [copied, setCopied] = useState(false)

  const handleSaveExternalDataToolModal = (
  ...
  const handleValidateBeforeSaveExternalDataToolModal = (
    ...
    for (let i = 0; i < existedExternalDataTools.length; i++) {
      if (
        existedExternalDataTools[i].variable === newExternalDataTool.variable
      ) {
        notify({
          type: "error",
          message: t("appDebug.varKeyError.keyAlreadyExists", {
            key: existedExternalDataTools[i].variable,
          }),
        })
        return false
      }
  ...
  const handleOpenExternalDataToolModal = (
    ...
    setShowExternalDataToolModal({
      ...
      onValidateBeforeSaveCallback: (newExternalDataTool: ExternalDataTool) =>
        handleValidateBeforeSaveExternalDataToolModal(
          newExternalDataTool,
          index,
        ),
    })
    ...
    <div className="mt-3 rounded-xl bg-gray-50 px-3">
      <div className="flex h-12 items-center">
        <div className="flex grow items-center">
          <div
            className={`group mr-1 flex h-6 w-6 items-center justify-center rounded-md ${externalDataToolsConfig.length && "hover:shadow-xs hover:bg-white"} `}
            ...
            {!!externalDataToolsConfig.length && (
              <RiArrowDownSLine
                className={`text-primary-600 hidden h-4 w-4 cursor-pointer group-hover:block ${expanded ? "rotate-180" : "rotate-0"}`}
              />
            )}
          </div>
        ...
        {!expanded && !!externalDataToolsConfig.length && (
          <>
            <div className="mr-3 text-xs text-gray-500">
              {t("appDebug.feature.tools.toolsInUse", {
                count: externalDataToolsConfig.length,
              })}
            </div>
          ...
          {externalDataToolsConfig.map((item, index: number) => (
            <div
              ...
              <div className="flex grow items-center">
                ...
                />
                <div className="mr-2 text-[13px] font-medium text-gray-800">
                  {item.label}
                </div>
                <TooltipPlus
                  popupContent={
