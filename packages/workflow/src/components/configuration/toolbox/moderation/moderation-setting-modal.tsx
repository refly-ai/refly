...
const ModerationSettingModal: FC<ModerationSettingModalProps> = ({
  ...
  return (
    <Modal
      isShow
      onClose={() => {}}
      className="!mt-14 !w-[640px] !max-w-none !p-8 !pb-6">
      ...
      <div className="py-2">
        ...
        <div className="grid grid-cols-3 gap-2.5">
          {providers.map(provider => (
            <div
              key={provider.key}
              className={`flex cursor-pointer items-center rounded-lg px-3 py-2 text-sm text-gray-900 ${localeData.type === provider.key ? "border-primary-400 border-[1.5px] bg-white shadow-sm" : "bg-gray-25 border border-gray-100"} ${localeData.type === "openai_moderation" && provider.key === "openai_moderation" && !openaiProviderConfiged && "opacity-50"} `}
              onClick={() => handleDataTypeChange(provider.key)}>
              <div
                className={`mr-2 h-4 w-4 rounded-full border ${localeData.type === provider.key ? "border-primary-600 border-[5px]" : "border border-gray-300"}`}
              />
              {provider.name}
            </div>
          ))}
        ...
        <div className="py-2">
          <div className="flex h-9 items-center justify-between">
            ...
            <a
              ...
              className="hover:text-primary-600 group flex items-center text-xs text-gray-500">
              <BookOpen01 className="group-hover:text-primary-600 mr-1 h-3 w-3 text-gray-500" />
      ...
      <ModerationContent
        ...
        showPreset={!(localeData.type === "api")}
      />
      <div className="mb-8 mt-1 text-xs font-medium text-gray-500">
      ...
      <div className="flex items-center justify-end">
        ...
        <Button
          ...
          onClick={handleSave}
          disabled={
            localeData.type === "openai_moderation" && !openaiProviderConfiged
          }>
