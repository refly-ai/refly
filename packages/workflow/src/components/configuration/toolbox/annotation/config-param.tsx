...
import type { EmbeddingModelConfig } from "@/components/annotation/type"
import {
  ...
  return (
    <div>
      <div className="flex items-center space-x-1">
        ...
        <TooltipPlus
          popupContent={
            <div className="max-w-[200px] text-[13px] font-medium leading-[18px] text-gray-800">
              {tooltip}
            </div>
          }>
...
const AnnotationReplyConfig: FC<Props> = ({
  ...
  const { annotationConfig } = useContext(ConfigContext)
  ...
  return (
    <>
      <Panel
        className="mt-4"
        headerIcon={<MessageFast className="h-4 w-4 text-[#444CE7]" />}
        title={t("appDebug.feature.annotation.title")}
        headerRight={
          <div className="flex items-center">
            <div
              className="flex h-7 cursor-pointer items-center space-x-1 rounded-md px-3 text-gray-700 hover:bg-gray-200"
              onClick={() => {
                setIsShowEdit(true)
              }}>
            ...
            <div
              className="ml-1 flex h-7 cursor-pointer items-center space-x-1 rounded-md px-3 text-xs font-medium leading-[18px] text-gray-700 hover:bg-gray-200"
              onClick={() => {
      ...
      {isShowEdit && (
        <ConfigParamModal
          ...
          onSave={async (embeddingModel, score) => {
            const annotationConfig = (await fetchAnnotationConfig(
              appId,
            )) as AnnotationReplyConfigType
            ...
            if (score !== annotationConfig.score_threshold) {
              ...
              if (isEmbeddingModelChanged) onScoreChange(score, embeddingModel)
              else onScoreChange(score)
            }
