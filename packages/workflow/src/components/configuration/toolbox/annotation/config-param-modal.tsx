...
const ConfigParamModal: FC<Props> = ({
  ...
  const [embeddingModel, setEmbeddingModel] = useState(
    oldAnnotationConfig.embedding_model
      ? {
          providerName:
            oldAnnotationConfig.embedding_model.embedding_provider_name,
  ...
  const handleSave = async () => {
    if (
      ...
      (embeddingModel.modelName === embeddingsDefaultModel?.model &&
      ...
      <div className="mt-6 space-y-3">
        <Item
          ...
          <ScoreSlider
            ...
            value={
              (annotationConfig.score_threshold ||
                ANNOTATION_DEFAULT.score_threshold) * 100
            }
            onChange={val => {
              setAnnotationConfig({
                ...annotationConfig,
