...
import { AnnotationEnableStatus, JobStatus } from "@/components/annotation/type"
...
const useAnnotationConfig = ({
  ...
  const [isShowAnnotationFullModal, setIsShowAnnotationFullModal] =
    useState(false)
  const [isShowAnnotationConfigInit, doSetIsShowAnnotationConfigInit] =
  ...
  const handleEnableAnnotation = async (
