...
import {
  ...
  checkHasQueryBlock,
} from "@/components/base/prompt-editor/constants"
...
const useAdvancedPromptConfig = ({
  ...
  const [completionPromptConfig, setCompletionPromptConfig] =
    useState<CompletionPromptConfig>(clone(DEFAULT_COMPLETION_PROMPT_CONFIG))

  const currentAdvancedPrompt = (() => {
    if (!isAdvancedPrompt) return []

    return modelModeType === ModelModeType.chat
      ? chatPromptConfig.prompt
      : completionPromptConfig.prompt
  })()

  const setCurrentAdvancedPrompt = (
    ...
    if (isUserChanged) onUserChangedPrompt()
  }

  const setConversationHistoriesRole = (
  ...
  const hasSetBlockStatus = (() => {
    ...
    if (modelModeType === ModelModeType.chat) {
      return {
        context: !!chatPromptConfig.prompt.find(p =>
          checkHasContextBlock(p.text),
        ),
        history: false,
   ...
   */
  const migrateToDefaultPrompt = async (
    isMigrateToCompetition?: boolean,
    toModelModeType?: ModelModeType,
  ) => {
    const mode = modelModeType
    const toReplacePrePrompt = prePrompt || ""
    if (!isAdvancedPrompt) {
      const { chat_prompt_config, completion_prompt_config, stop } =
      ...
      if (modelModeType === ModelModeType.chat) {
        ...
        const newPromptConfig = produce(completion_prompt_config, draft => {
          draft.prompt.text = draft.prompt.text.replace(
            PRE_PROMPT_PLACEHOLDER_TEXT,
            toReplacePrePrompt,
          )
        })
    ...
    if (isMigrateToCompetition) {
      const { completion_prompt_config, chat_prompt_config, stop } =
        await fetchPromptTemplate({
          appMode,
      ...
      if (toModelModeType === ModelModeType.completion) {
        const newPromptConfig = produce(completion_prompt_config, draft => {
          if (!completionPromptConfig.prompt?.text)
            ...
            draft.prompt.text = completionPromptConfig.prompt?.text.replace(
              PRE_PROMPT_PLACEHOLDER_TEXT,
              toReplacePrePrompt,
            )

          if (
            ["advanced-chat", "agent-chat", "chat"].includes(appMode) &&
            completionPromptConfig.conversation_histories_role
              .assistant_prefix &&
            completionPromptConfig.conversation_histories_role.user_prefix
          )
            draft.conversation_histories_role =
              completionPromptConfig.conversation_histories_role
        })
        setCompletionPromptConfig(newPromptConfig)
