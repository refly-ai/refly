...
import ChatInput from "@/components/base/chat/chat/chat-input"
import type { VisionFile } from "@/components/base/chat/types"
import { useDebugConfigurationContext } from "@/context/debug-configuration"

const DebugWithMultipleModel = () => {
  const { mode, speechToTextConfig, visionConfig } =
    useDebugConfigurationContext()
  const { multipleModelConfigs, checkCanSend } =
    useDebugWithMultipleModelContext()
  const { eventEmitter } = useEventEmitterContextContext()
  const isChatMode = mode === "chat" || mode === "agent-chat"

  const handleSend = useCallback(
    (message: string, files?: VisionFile[]) => {
      ...
      <div
        ...
        {multipleModelConfigs.map((modelConfig, index) => (
          <DebugItem
            ...
            className={`absolute left-6 top-0 min-h-[200px] ${twoLine && index === 0 && "mr-2"} ${threeLine && (index === 0 || index === 1) && "mr-2"} ${fourLine && (index === 0 || index === 2) && "mr-2"} ${fourLine && (index === 0 || index === 1) && "mb-2"} `}
            style={{
              width: size.width,
              height: size.height,
