...
import ConfirmModal from "@/components/base/confirm/common"
...
const ConfigVar: FC<IConfigVarProps> = ({
  promptVariables,
  readonly,
  ...
  const updatePromptVariableItem = (payload: InputVar) => {
    ...
    const newPromptVariables = produce(promptVariables, draft => {
      ...
      if (payload.type === InputVarType.textInput)
        draft[currIndex].max_length =
          draft[currIndex].max_length || DEFAULT_VALUE_MAX_LEN

      if (payload.type !== InputVarType.select) delete draft[currIndex].options
    })
  ...
  const updatePromptKey = (index: number, newKey: string) => {
    ...
    conflictTimer = setTimeout(() => {
      const isKeyExists = promptVariables.some(
        item => item.key?.trim() === newKey.trim(),
      )
      if (isKeyExists) {
        Toast.notify({
          type: "error",
  ...
  const handleOpenExternalDataToolModal = (
    ...
    setShowExternalDataToolModal({
      ...
      onCancelCallback: () => {
        if (!key)
          onPromptVariablesChange?.(
            promptVariables.filter((_, i) => i !== index),
          )
      },
      onValidateBeforeSaveCallback: (newExternalDataTool: ExternalDataTool) => {
        for (let i = 0; i < promptVariables.length; i++) {
          if (
            promptVariables[i].key === newExternalDataTool.variable &&
            i !== index
          ) {
            Toast.notify({
              type: "error",
  ...
  const [isShowEditModal, { setTrue: showEditModal, setFalse: hideEditModal }] =
    useBoolean(false)
  ...
  return (
    <Panel
      className="mt-4"
      headerIcon={<VarIcon className="text-primary-500 h-4 w-4" />}
      title={
        <div className="flex items-center">
          ...
          {!readonly && (
            <Tooltip
              htmlContent={
                <div className="w-[180px]">{t("appDebug.variableTip")}</div>
              }
      ...
      headerRight={
        !readonly ? <SelectVarType onChange={handleAddVar} /> : null
      }>
      {!hasVar && (
      ...
      {hasVar && (
        <div className="overflow-x-auto rounded-lg border border-gray-200 bg-white">
          <table
            className={`${s.table} w-full min-w-[440px] max-w-full border-collapse rounded-lg border-0 text-sm`}>
            ...
            <tbody className="text-gray-700">
              {promptVariables.map(
                (
                  { key, name, type, required, config, icon, icon_background },
                  index,
                ) => (
                  <tr key={index} className="h-9 leading-9">
                    <td className="w-[160px] border-b border-gray-100 pl-3">
                      <div className="flex items-center space-x-1">
                        <IconTypeIcon
                          type={type as IInputTypeIconProps["type"]}
                        ...
                        {!readonly ? (
                          <input
                            ...
                            value={key}
                            onChange={e =>
                              updatePromptKey(index, e.target.value)
                            }
                            onBlur={e =>
                              updatePromptNameIfNameEmpty(index, e.target.value)
                            }
                            maxLength={getMaxVarNameLength(name)}
                            className="block h-6 w-full rounded-md border-0 py-1.5 leading-6 text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-inset focus:ring-gray-200"
                          />
                        ) : (
                          <div className="h-6 text-[13px] leading-6 text-gray-700">
                            {key}
                          </div>
                        )}
                      </div>
                    ...
                    <td className="border-b border-gray-100 py-1">
                      {!readonly ? (
                        <input
                          ...
                          value={name}
                          onChange={e =>
                            updatePromptVariable(key, "name", e.target.value)
                          }
                          maxLength={getMaxVarNameLength(name)}
                          className="block h-6 w-full rounded-md border-0 py-1.5 leading-6 text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-inset focus:ring-gray-200"
                        />
                      ...
                      <>
                        ...
                        <td className="w-20 border-b border-gray-100">
                          <div className="flex h-full items-center space-x-1">
                            <div
                              className="h-6 w-6 cursor-pointer rounded-md p-1 hover:bg-black/5"
                              onClick={() =>
                                handleConfig({
                                  type,
                                  key,
      ...
      {isShowDeleteContextVarModal && (
        <ConfirmModal
          ...
          desc={
            t(
              "appDebug.feature.dataSet.queryVariable.deleteContextVarTip",
            ) as string
          }
