...
import type { InputVar, MoreInfo } from "@/components/workflow/types"
import Modal from "@/components/base/modal"
import Switch from "@/components/base/switch"
import { ChangeType, InputVarType } from "@/components/workflow/types"
...
const inputClassName =
  "w-full px-3 text-sm leading-9 text-gray-900 border-0 rounded-lg grow h-9 bg-gray-100 focus:outline-none focus:ring-1 focus:ring-inset focus:ring-gray-200"
...
const ConfigModal: FC<IConfigModalProps> = ({
  ...
  const handleConfirm = () => {
    ...
    if (!tempPayload.variable) {
      Toast.notify({
        type: "error",
        message: t("appDebug.variableConig.errorMsg.varNameRequired"),
      })
      return
    }
    ...
    if (!tempPayload.label) {
      Toast.notify({
        type: "error",
        message: t("appDebug.variableConig.errorMsg.labelNameRequired"),
      })
      return
    }
    if (isStringInput || type === InputVarType.number) {
      ...
      if (options?.length === 0) {
        Toast.notify({
          type: "error",
          message: t("appDebug.variableConig.errorMsg.atLeastOneOption"),
        })
        return
      }
      ...
      if (hasRepeatedItem) {
        Toast.notify({
          type: "error",
          message: t("appDebug.variableConig.errorMsg.optionRepeat"),
        })
        return
      }
      onConfirm(tempPayload, moreInfo)
        ...
        <div className="space-y-2">
          <Field title={t("appDebug.variableConig.fieldType")}>
            <div className="flex space-x-2">
              <SelectTypeItem
                ...
                onClick={() =>
                  handlePayloadChange("type")(InputVarType.textInput)
                }
              />
              <SelectTypeItem
                ...
                onClick={() =>
                  handlePayloadChange("type")(InputVarType.paragraph)
                }
              />
              <SelectTypeItem
                type={InputVarType.select}
                selected={type === InputVarType.select}
                onClick={() => handlePayloadChange("type")(InputVarType.select)}
              />
              <SelectTypeItem
                type={InputVarType.number}
                selected={type === InputVarType.number}
                onClick={() => handlePayloadChange("type")(InputVarType.number)}
              />
            </div>
          </Field>
          ...
          <Field title={t("appDebug.variableConig.required")}>
            <Switch
              ...
              onChange={handlePayloadChange("required")}
            />
          </Field>
        </div>
      </div>
