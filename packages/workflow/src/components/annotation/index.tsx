...
import ConfigParamModal from "@/components/app/configuration/toolbox/annotation/config-param-modal"
...
const Annotation: FC<Props> = ({ appDetail }) => {
  const { t } = useTranslation()
  const [isShowEdit, setIsShowEdit] = React.useState(false)
  const [annotationConfig, setAnnotationConfig] =
    useState<AnnotationReplyConfig | null>(null)
  ...
  const [isShowAnnotationFullModal, setIsShowAnnotationFullModal] =
    useState(false)
  const ensureJobCompleted = async (
    jobId: string,
    status: AnnotationEnableStatus,
  ) => {
    let isCompleted = false
    while (!isCompleted) {
      const res: any = await queryAnnotationJobStatus(
        appDetail.id,
        status,
        jobId,
      )
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex grow flex-col py-4">
              ...
              <>
                <div
                  className={cn(
                    ...
                    "flex h-7 items-center space-x-1 rounded-lg border border-gray-200 pl-2",
                  )}>
                  <div className="text-[13px] font-medium leading-[18px] text-gray-900">
                    {t("appAnnotation.name")}
                  </div>
                  <Switch
                  ...
                  {annotationConfig?.enabled && (
                    <div className="flex items-center pl-1.5">
                      <div className="mr-1 h-3.5 w-[1px] shrink-0 bg-gray-200"></div>
                      <div
                        className={`flex h-7 w-7 shrink-0 items-center justify-center text-xs font-medium text-gray-700`}
                        onClick={() => {
                          setIsShowEdit(true)
                        }}>
                        <div className="flex h-6 w-6 cursor-pointer items-center justify-center rounded-md hover:bg-gray-200">
                          <Settings04 className="h-4 w-4" />
                        </div>
                      </div>
                    </div>
                  )}
                </div>
                <div className="mx-3 h-3.5 w-[1px] shrink-0 bg-gray-200"></div>
              </>
            )}
        ...
        {isLoading ? (
          ...
          <List list={list} onRemove={handleRemove} onView={handleView} />
        ) : (
