...
const HeaderOptions: FC<Props> = ({
  ...
  const listTransformer = (list: AnnotationItemBasic[]) =>
      ...
      <div className="w-full py-1">
        ...
        <Menu as="div" className="relative h-full w-full">
          ...
          <Transition
            ...
            <Menu.Items
              className={cn(
                `absolute top-[1px] z-10 min-w-[100px] origin-top-right divide-y divide-gray-100 rounded-xl border-[0.5px] border-gray-200 bg-white py-1`,
                s.popup,
              )}>
              <CSVDownloader
                type={Type.Link}
              ...
              <button
                disabled={annotationUnavailable}
                className={cn(s.actionItem, "!border-0")}
                onClick={JSONLOutput}>
  ...
  return (
    <div className="flex space-x-2">
      <Button
        ...
        className="flex !h-8 items-center space-x-2 !px-3 !text-[13px]">
      ...
      <CustomPopover
        ...
        btnClassName={open =>
          cn(
            open
              ? "border-gray-300 !bg-gray-100 !shadow-none"
              : "border-gray-200",
            s.actionIconWrapper,
          )
        }
