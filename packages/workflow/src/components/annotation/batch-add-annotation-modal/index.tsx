...
import {
  ...
  checkAnnotationBatchImportProgress,
...
const BatchModal: FC<IBatchModalProps> = ({
  ...
  const checkProcess = async (jobID: string) => {
    try {
      ...
      if (res.job_status === ProcessStatus.ERROR)
        notify({
          type: "error",
          message: `${t("appAnnotation.batchModal.runError")}`,
        })
      if (res.job_status === ProcessStatus.COMPLETED) {
        notify({
          type: "success",
          message: `${t("appAnnotation.batchModal.completed")}`,
        })
        onAdded()
        onCancel()
      }
      ...
      notify({
        type: "error",
        message: `${t("appAnnotation.batchModal.runError")}${"message" in e ? `: ${e.message}` : ""}`,
      })
    }
  }
  ...
  return (
    <Modal
      isShow={isShow}
      onClose={() => {}}
      className="!max-w-[520px] !rounded-xl px-8 py-6">
      <div className="relative pb-1 text-xl font-medium leading-[30px] text-gray-900">
