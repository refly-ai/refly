...
const CSV_TEMPLATE_EN = [["segment content"], ["content1"], ["content2"]]
...
const CSVDownload: FC<{ docForm: DocForm }> = ({ docForm }) => {
  ...
  const getTemplate = () => {
    if (locale === LanguagesSupported[1]) {
      if (docForm === DocForm.QA) return CSV_TEMPLATE_QA_CN
      return CSV_TEMPLATE_CN
    }
    if (docForm === DocForm.QA) return CSV_TEMPLATE_QA_EN
            ...
            <tbody className="text-gray-700">
              <tr>
                <td className="h-9 border-b border-gray-100 pl-3 pr-2 text-[13px]">
                  {t("datasetDocuments.list.batchModal.question")} 1
                </td>
                <td className="h-9 border-b border-gray-100 pl-3 pr-2 text-[13px]">
                  {t("datasetDocuments.list.batchModal.answer")} 1
                </td>
              </tr>
              <tr>
                ...
                <td className="h-9 pl-3 pr-2 text-[13px]">
                  {t("datasetDocuments.list.batchModal.answer")} 2
                </td>
              </tr>
            </tbody>
          </table>
