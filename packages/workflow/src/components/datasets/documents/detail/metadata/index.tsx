...
import AutoHeightTextarea from "@/components/base/auto-height-textarea"
...
const Metadata: FC<IMetadataProps> = ({ docDetail, loading, onUpdate }) => {
        ...
        <div className={s.operationWrapper}>
            ...
            {CUSTOMIZABLE_DOC_TYPES.map((type, index) => {
              ...
              return (
                <Radio
                  key={index}
                  value={type}
                  className={`${s.radio} ${currValue === type ? "shadow-none" : ""}`}>
          ...
          {documentType && (
            <div className={s.opBtnWrapper}>
              ...
              <Button
                onClick={cancelDocType}
                className={`${s.opBtn} ${s.opCancelBtn}`}>
  ...
  const onCancel = () => {
    ...
    if (!doc_type) setShowDocTypes(true)
  }
        ...
        <>
          ...
          {!editStatus ? (
            <div className={s.documentTypeShow}>
              <TypeIcon
                iconName={metadataMap[doc_type || "book"]?.iconName || ""}
            ...
            <div className={s.documentTypeShow}>
              {metadataParams.documentType && (
                <>
                  <TypeIcon
                    iconName={
                      metadataMap[metadataParams.documentType || "book"]
                        .iconName || ""
                    }
                    className={s.iconShow}
                  />
