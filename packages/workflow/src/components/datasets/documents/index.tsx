...
import { NotionPageSelectorModal } from "@/components/base/notion-page-selector"
import type { NotionPage } from "@/models/common"
import type { CreateDocumentReq } from "@/models/datasets"
import { DataSourceType } from "@/models/datasets"
import RetryButton from "@/components/base/retry-button"
  ...
  return (
    <div className={s.emptyWrapper}>
      <div className={s.emptyElement}>
        ...
        {type === "upload" && canAdd && (
          <Button onClick={onClick} className={s.addFileBtn}>
...
const Documents: FC<IDocumentsProps> = ({ datasetId }) => {
  ...
  const [notionPageSelectorModalVisible, setNotionPageSelectorModalVisible] =
    useState(false)
  ...
  const documentsWithProgress = useMemo(() => {
    ...
    const documentsData = documentsRes?.data?.map(documentItem => {
      const { indexing_status, completed_segments, total_segments } =
        documentItem
      const isEmbeddinged =
    ...
    if (completedNum === documentsRes?.data?.length) setTimerCanRun(false)
    return {
      ...documentsRes,
      data: documentsData,
    }
  ...
  const documentsList = isDataSourceNotion
    ? documentsWithProgress?.data
    : documentsRes?.data
