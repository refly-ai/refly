...
import { RiMoreFill, RiQuestionLine } from "@remixicon/react"
...
import type { IndicatorProps } from "@/components/header/indicator"
...
import NotionIcon from "@/components/base/notion-icon"
...
export const useIndexStatus = () => {
  ...
  return {
    queuing: {
      color: "orange",
      text: t("datasetDocuments.list.status.queuing"),
    }, // waiting
    indexing: {
      color: "blue",
      text: t("datasetDocuments.list.status.indexing"),
    ...
    available: {
      color: "green",
      text: t("datasetDocuments.list.status.available"),
    }, // completed，archived = false，enabled = true
    enabled: {
      color: "green",
      text: t("datasetDocuments.list.status.enabled"),
    }, // completed，archived = false，enabled = true
    disabled: {
      color: "gray",
      text: t("datasetDocuments.list.status.disabled"),
    }, // completed，archived = false，enabled = false
    archived: {
      color: "gray",
      text: t("datasetDocuments.list.status.archived"),
...
export const StatusItem: FC<{
  ...
  return (
    <div
      ...
      <Indicator
        color={
          DOC_INDEX_STATUS_MAP[localStatus]?.color as IndicatorProps["color"]
        }
        className={reverse ? "ml-2" : "mr-2"}
      />
      <span className={cn("text-sm text-gray-700", textCls)}>
      ...
      {errorMessage && (
        <Tooltip
          ...
          <RiQuestionLine className="ml-1 h-[14px] w-[14px] text-gray-700" />
        </Tooltip>
      )}
    </div>
  )
  ...
  const onOperate = async (operationName: OperationName) => {
    ...
    switch (operationName) {
      ...
      case "disable":
        ...
        break
      case "sync":
        if (data_source_type === "notion_import") opApi = syncDocument
        else opApi = syncWebsite
        ...
        <>
          {archived ? (
            <Tooltip
              ...
              content={
                t("datasetDocuments.list.action.enableWarning") as string
              }
            ...
            <div className="w-full py-1">
                ...
                <>
                  <div className="mx-4 flex items-center justify-between pt-2">
                    <span className={cn(s.actionName, "font-medium")}>
                      {!archived && enabled
                        ? t("datasetDocuments.list.index.enable")
                        : t("datasetDocuments.list.index.disable")}
                    </span>
                    <Tooltip
                      ...
                      <div>
                        <Switch
                          ...
                          onChange={v =>
                            !archived && handleSwitch(v ? "enable" : "disable")
                          }
                          disabled={archived}
                          size="md"
                        />
                  ...
                  <div className="mx-4 pb-1 pt-0.5 text-xs text-gray-500">
                    {!archived && enabled
                      ? t("datasetDocuments.list.index.enableTip")
                      : t("datasetDocuments.list.index.disableTip")}
                  </div>
                  <Divider />
                </>
              )}
                ...
                <>
                  <div
                    ...
                    <span className={s.actionName}>
                      {t("datasetDocuments.list.table.rename")}
                    </span>
                  </div>
                  <div
                    className={s.actionItem}
                  ...
                  {["notion_import", DataSourceType.WEB].includes(
                    data_source_type,
                  ) && (
                    <div
                      className={s.actionItem}
                      ...
                      <SyncIcon />
                      <span className={s.actionName}>
                        {t("datasetDocuments.list.action.sync")}
                      </span>
                    </div>
                  )}
              ...
              {archived && (
                <div
                  ...
                  <ArchiveIcon />
                  <span className={s.actionName}>
                    {t("datasetDocuments.list.action.unarchive")}
                  </span>
                </div>
              )}
              <div
                ...
                <TrashIcon
                  className={
                    "h-4 w-4 stroke-current stroke-2 text-gray-500 group-hover:text-red-500"
                  }
                />
...
export const renderTdValue = (
  ...
  return (
    <div
      className={cn(
        isEmptyStyle ? "text-gray-400" : "text-gray-700",
        s.tdValue,
      )}>
      {value ?? "-"}
    </div>
...
const DocumentList: FC<IDocumentListProps> = ({
  ...
  return (
    <div className="h-full w-full overflow-x-auto">
      <table
        className={`mt-3 w-full min-w-[700px] max-w-full border-collapse border-0 text-sm ${s.documentTable}`}>
        <thead className="h-8 border-b border-gray-200 text-xs font-medium uppercase leading-8 text-gray-500">
          <tr>
            ...
            <td className="w-44">
              <div className="flex items-center justify-between">
                ...
                <ArrowDownIcon
                  className={cn(
                    "h-3 w-3 cursor-pointer stroke-current stroke-2",
        ...
        <tbody className="text-gray-700">
          {localDocs.map(doc => {
            ...
            const fileType = isFile
              ? doc.data_source_detail_dict?.upload_file.extension
              : ""
            return (
              <tr
                key={doc.id}
                ...
                <td className="text-left align-middle text-xs text-gray-500">
                  {doc.position}
                </td>
                <td>
                  <div className="group flex items-center justify-between">
                    <span className={s.tdValue}>
                      {doc?.data_source_type === DataSourceType.NOTION && (
                        <NotionIcon
                          className="-mt-[3px] mr-1.5 inline-flex align-middle"
                          type="page"
                          src={doc.data_source_info.notion_page_icon}
                        />
                      )}
                      {doc?.data_source_type === DataSourceType.FILE && (
                        <div
                          className={cn(
                            s[
                              `${doc?.data_source_info?.upload_file?.extension ?? fileType}Icon`
                            ],
                            s.commonIcon,
                            "mr-1.5",
                          )}></div>
                      ...
                      {doc?.data_source_type === DataSourceType.WEB && (
                        <Globe01 className="-mt-[3px] mr-1.5 inline-flex align-middle" />
                      )}
                      {doc.name}
                    </span>
