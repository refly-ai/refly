...
import { useModelListAndDefaultModel } from "@/components/header/account-setting/model-provider-page/hooks"
...
const RetrievalParamConfig: FC<Props> = ({ type, value, onChange }) => {
  const { t } = useTranslation()
  const canToggleRerankModalEnable = type !== RETRIEVE_METHOD.hybrid
  const isEconomical = type === RETRIEVE_METHOD.invertedIndex
  const { defaultModel: rerankDefaultModel, modelList: rerankModelList } =
  ...
  return (
    <div>
      {!isEconomical && (
        <div>
          <div className="flex h-8 items-center space-x-2 text-[13px] font-medium text-gray-900">
          ...
          <ModelSelector
            ...
            readonly={
              !value.reranking_enable && type !== RETRIEVE_METHOD.hybrid
            }
            onSelect={v => {
              onChange({
                ...value,
      ...
      <div
        className={cn(!isEconomical && "mt-4", "space-between flex space-x-6")}>
        <TopKItem
          className="grow"
          value={value.top_k}
          onChange={(_key, v) => {
