...
const FireCrawl: FC<Props> = ({
        ...
        if (res.status === "completed") {
          return {
            ...
            data: {
              ...
              total: Math.min(
                res.total,
                parseFloat(crawlOptions.limit as string),
              ),
            },
          }
        }
    ...
    async (url: string) => {
      ...
      try {
        ...
        const { isError, data, errorMessage } =
          await waitForCrawlFinished(jobId)
        if (isError) {
  ...
  return (
    <div>
      ...
      <div className={cn("mt-2 rounded-xl border border-gray-200 p-4 pb-0")}>
          ...
          <div className="relative left-[-16px] mt-3 w-[calc(100%_+_32px)] rounded-b-xl">
            ...
            {isCrawlFinished && !showError && (
              <CrawledResult
                ...
                onPreview={onPreview}
                usedTime={
                  parseFloat(crawlResult?.time_consuming as string) || 0
                }
              />
            )}
          </div>
