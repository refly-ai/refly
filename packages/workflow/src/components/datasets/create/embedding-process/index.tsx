...
import { FieldInfo } from "@/components/datasets/documents/detail/metadata"
...
import {
  fetchIndexingStatusBatch as doFetchIndexingStatus,
  fetchIndexingEstimateBatch,
  fetchProcessRule,
} from "@/service/datasets"
import { DataSourceType } from "@/models/datasets"
import NotionIcon from "@/components/base/notion-icon"
import PriorityLabel from "@/components/billing/priority-label"
import { Plan } from "@/components/billing/type"
import { ZapFast } from "@/components/base/icons/src/vender/solid/general"
...
const RuleDetail: FC<{ sourceData?: ProcessRuleResponse }> = ({
  sourceData,
}) => {
  ...
  return (
    <div className="flex flex-col pb-10 pt-8 first:mt-0">
...
const EmbeddingProcess: FC<Props> = ({
  datasetId,
  batchId,
  documents = [],
  indexingType,
  ...
  const [indexingStatusBatchDetail, setIndexingStatusDetail] = useState<
    IndexingStatusResponse[]
  >([])
  ...
  const startQueryStatus = async () => {
    if (isStopQueryRef.current) return

    try {
      const indexingStatusBatchDetail = await fetchIndexingStatus()
      const isCompleted = indexingStatusBatchDetail.every(
        indexingStatusDetail =>
          ["completed", "error", "paused"].includes(
    ...
    <>
        ...
        <div className={s.cost}>
          {indexingType === "high_quality" && (
            <div className="flex items-center">
              ...
              {t("datasetDocuments.embedding.highQuality")} ·{" "}
              {t("datasetDocuments.embedding.estimate")}
              <span className={s.tokens}>
                {formatNumber(indexingEstimateDetail?.tokens || 0)}
              </span>
              tokens (
              <span className={s.price}>
                ${formatNumber(indexingEstimateDetail?.total_price || 0)}
              </span>
              )
            </div>
          )}
          {indexingType === "economy" && (
            <div className="flex items-center">
              ...
              {t("datasetDocuments.embedding.economy")} ·{" "}
              {t("datasetDocuments.embedding.estimate")}
            ...
            <div className={`${s.info} grow`}>
              {getSourceType(indexingStatusDetail.id) ===
                ...
                <div
                  className={cn(
                    s.fileIcon,
                    s[getFileType(getSourceName(indexingStatusDetail.id))],
                  )}
                />
              )}
              {getSourceType(indexingStatusDetail.id) ===
              ...
              <div
                className={`${s.name} truncate`}
                title={getSourceName(indexingStatusDetail.id)}>
                {getSourceName(indexingStatusDetail.id)}
              </div>
              {enableBilling && <PriorityLabel />}
            </div>
            <div className="shrink-0">
              {isSourceEmbedding(indexingStatusDetail) && (
                <div
                  className={
                    s.percent
                  }>{`${getSourcePercent(indexingStatusDetail)}%`}</div>
              )}
              {indexingStatusDetail.indexing_status === "error" &&
                indexingStatusDetail.error && (
                  <TooltipPlus
                    ...
                    <div
                      className={cn(s.percent, s.error, "flex items-center")}>
                      Error
                      <RiErrorWarningFill className="ml-1 h-4 w-4" />
                    </div>
                  </TooltipPlus>
                )}
              {indexingStatusDetail.indexing_status === "error" &&
      ...
      <div className="mt-10 flex items-center gap-2">
        <Button className="w-fit" variant="primary" onClick={navToDocumentList}>
          ...
          <ArrowRightIcon className="ml-2 h-4 w-4 stroke-current stroke-1" />
        </Button>
      </div>
    </>
  )
