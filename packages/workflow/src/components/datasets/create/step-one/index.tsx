...
import { NotionPageSelector } from "@/components/base/notion-page-selector"
import { useDatasetDetailContext } from "@/context/dataset-detail"
import { useProviderContext } from "@/context/provider-context"
import VectorSpaceFull from "@/components/billing/vector-space-full"
...
export const NotionConnector = ({ onSetting }: NotionConnectorProps) => {
  ...
  return (
    <div className={s.notionConnectionTip}>
      ...
      <Button className="h-8" variant="primary" onClick={onSetting}>
...
const StepOne = ({
  ...
  const shouldShowDataSourceTypeList =
    !datasetId || (datasetId && !dataset?.data_source_type)
  ...
  const isShowVectorSpaceFull =
    (allFileLoaded || hasNotin) && isVectorSpaceFull && enableBilling
  const notSupportBatchUpload = enableBilling && plan.type === "sandbox"
  const nextDisabled = useMemo(() => {
    if (!files.length) return true
    if (files.some(file => !file.file.id)) return true
    if (isShowVectorSpaceFull) return true
  ...
  return (
    <div className="flex h-full w-full">
          ...
          {shouldShowDataSourceTypeList && (
            <div className="mb-8 flex flex-wrap items-center gap-y-4">
              <div
                ...
                onClick={() => {
                  if (dataSourceTypeDisable) return
      ...
      {currentFile && (
        <FilePreview file={currentFile} hidePreview={hideFilePreview} />
      )}
      {currentNotionPage && (
        <NotionPagePreview
