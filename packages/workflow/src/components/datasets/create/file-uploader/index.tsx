...
const FileUploader = ({
  ...
  const isValid = useCallback(
    (file: File) => {
      ...
      if (!isValidSize)
        notify({
          type: "error",
          message: t("datasetCreation.stepOne.uploader.validation.size", {
          ...
          notify({
            ...
            message:
              e?.response?.code === "forbidden"
                ? e?.response?.message
                : t("datasetCreation.stepOne.uploader.failed"),
          })
  ...
  const initialUpload = useCallback(
    (files: File[]) => {
      ...
      if (
        files.length + fileList.length > FILES_NUMBER_LIMIT &&
        !IS_CE_EDITION
      ) {
        notify({
          type: "error",
          message: t(
            ...
            { filesNumber: FILES_NUMBER_LIMIT },
          ),
        })
        return false
      }
    ...
    <div className={s.fileUploader}>
      ...
      {!hideUpload && (
        <div ref={dropRef} className={cn(s.uploader, dragging && s.dragging)}>
          <div className="min-h-6 mb-2 flex items-center justify-center">
