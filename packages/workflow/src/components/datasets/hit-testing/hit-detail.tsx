...
const ScatterChart: FC<IScatterChartProps> = ({ data, curr }) => {
  ...
  return <ReactECharts option={option} style={{ height: 380, width: 430 }} />
}
...
const HitDetail: FC<IHitDetailProps> = ({ segInfo, vectorInfo }) => {
    ...
    if (segInfo?.answer) {
      return (
        <>
          ...
          <div className="text-md mb-4 text-gray-800">{segInfo.content}</div>
  ...
  return (
    <div className="flex flex-row overflow-x-auto">
      <div className="bg-gray-25 min-w-[300px] flex-1 p-6">
      ...
      <div className="flex-1 bg-white p-6">
        ...
        <div
          className={cn(s.numberInfo, "mt-1 w-[400px] truncate text-gray-700")}>
          {segInfo?.index_node_hash}
        </div>
        <ScatterChart
          data={vectorInfo?.points || []}
          curr={vectorInfo?.curr || []}
        />
      </div>
    </div>
  )
