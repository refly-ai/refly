...
import RunOnce from "@/components/share/text-generation/run-once"
import {
...
import Res from "@/components/share/text-generation/result"
import SavedItems from "@/components/app/text-generate/saved-items"
...
const TextGeneration: FC<IMainProps> = ({
  ...
  const allTaskRuned = allTaskList.every(task =>
  ...
  const handleCompleted = (
    ...
    const hadRunedTaskNum =
      1 +
      allTasklistLatest.filter(task =>
        [TaskStatus.completed, TaskStatus.failed].includes(task.status),
      ).length
    const needToAddNextGroupTask =
      getCurrGroupNum() !== hadRunedTaskNum &&
      pendingTaskList.length > 0 &&
      (hadRunedTaskNum % GROUP_SIZE === 0 ||
        allTasklistLatest.length - hadRunedTaskNum < GROUP_SIZE)
        ...
        <div
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between">
              ...
              {!isPC && (
                <Button className="ml-2 shrink-0" onClick={showResSidebar}>
                ...
                ? [
                    {
                      ...
                      extra:
                        savedMessages.length > 0 ? (
                          <div className="flext ml-1 h-5 items-center rounded-md border border-gray-200 px-1.5 text-xs font-medium text-gray-500">
          ...
          <div
            ...
            <div className="">
              © {siteInfo.copyright || siteInfo.title}{" "}
