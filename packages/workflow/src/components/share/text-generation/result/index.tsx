...
import TextGenerationRes from "@/components/app/text-generate/item"
import NoData from "@/components/share/text-generation/no-data"
...
import type { WorkflowProcess } from "@/components/chat/types"
...
const Result: FC<IResultProps> = ({
  ...
  const [workflowProcessData, doSetWorkflowProccessData] =
    useState<WorkflowProcess>()
      ...
      sendWorkflowMessage(
        data,
        {
          ...
          onIterationStart: ({ data }) => {
            setWorkflowProccessData(
              produce(getWorkflowProccessData()!, draft => {
                draft.expand = true
                draft.tracing!.push({
                  ...data,
            ...
            if (!data.outputs) {
              ...
              if (isStringOutput) {
                setWorkflowProccessData(
                  produce(getWorkflowProccessData()!, draft => {
                    draft.resultText =
                      data.outputs[Object.keys(data.outputs)[0]]
                  }),
                )
              }
            }
  ...
  return (
    <div className={cn(isNoData && !isCallBatchAPI && "h-full")}>
      ...
      {isCallBatchAPI && (
        <div className="mt-2">{renderTextGenerationRes()}</div>
      )}
    </div>
  )
}
