...
const PublishWithMultipleModel: FC<PublishWithMultipleModelProps> = ({
  ...
  multipleModelConfigs.forEach(item => {
    const provider = textGenerationModelList.find(
      model => model.provider === item.provider,
    )
  ...
  const handleToggle = () => {
    if (validModelConfigs.length) setOpen(v => !v)
  }
  ...
  return (
    <PortalToFollowElem
      ...
      <PortalToFollowElemContent className="z-50 mt-1 w-[288px]">
        <div className="rounded-lg border-[0.5px] border-gray-200 bg-white p-1 shadow-lg">
          ...
          {validModelConfigs.map((item, index) => (
            <div
              key={item.id}
              className="flex h-8 cursor-pointer items-center rounded-lg px-3 text-sm text-gray-500 hover:bg-gray-100"
              ...
              />
              <div
                className="ml-1 truncate text-gray-700"
