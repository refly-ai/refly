...
const QueryBlockComponent: FC<QueryBlockComponentProps> = ({ nodeKey }) => {
  ...
  return (
    <div
      className={`inline-flex h-6 items-center rounded-[5px] border border-transparent bg-[#FFF6ED] pl-1 pr-0.5 hover:bg-[#FFEAD5] ${isSelected && "!border-[#FD853A]"} `}
      ref={ref}>
      <UserEdit02 className="mr-1 h-[14px] w-[14px] text-[#FD853A]" />
      <div className="text-xs font-medium text-[#EC4A0A] opacity-60">
        {"{{"}
      </div>
