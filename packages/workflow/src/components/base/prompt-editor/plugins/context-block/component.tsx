...
const ContextBlockComponent: FC<ContextBlockComponentProps> = ({
  ...
  return (
    <div
      className={`group inline-flex h-6 items-center rounded-[5px] border border-transparent bg-[#F4F3FF] pl-1 pr-0.5 text-[#6938EF] hover:bg-[#EBE9FE] ${open ? "bg-[#EBE9FE]" : "bg-[#F4F3FF]"} ${isSelected && "!border-[#9B8AFB]"} `}
      ref={ref}>
      ...
      {!canNotAddContext && (
        <PortalToFollowElem
          ...
          <PortalToFollowElemTrigger ref={triggerRef}>
            <div
              className={`flex h-[18px] w-[18px] cursor-pointer items-center justify-center rounded text-[11px] font-semibold ${open ? "bg-[#6938EF] text-white" : "group-hover:shadow-xs bg-white/50 group-hover:bg-white"} `}>
            ...
            <div className="w-[360px] rounded-xl bg-white shadow-lg">
              <div className="p-4">
                <div className="mb-2 text-xs font-medium text-gray-500">
                  {t("common.promptEditor.context.modal.title", {
