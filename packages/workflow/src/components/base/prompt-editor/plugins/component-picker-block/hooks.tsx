...
export const useVariableOptions = (
  ...
  const options = useMemo(() => {
    const baseOptions = (variableBlock?.variables || []).map(item => {
      return new VariableOption(item.value, {
        icon: <BracketsX className="h-[14px] w-[14px] text-[#2970FF]" />,
        onSelect: () => {
  ...
  const addOption = useMemo(() => {
    return new VariableOption(t("common.promptEditor.variable.modal.add"), {
      icon: <BracketsX className="mr-2 h-[14px] w-[14px] text-[#2970FF]" />,
      onSelect: () => {
        editor.update(() => {
...
export const useExternalToolOptions = (
  ...
  const options = useMemo(() => {
    const baseToolOptions = (externalToolBlockType?.externalTools || []).map(
      item => {
  ...
  const addOption = useMemo(() => {
    return new VariableOption(t("common.promptEditor.variable.modal.addTool"), {
      icon: <Tool03 className="mr-2 h-[14px] w-[14px] text-[#444CE7]" />,
      extraElement: <ArrowUpRight className="h-3 w-3 text-gray-400" />,
      onSelect: () => {
