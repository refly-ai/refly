...
const ChooseFeature = ({ onChange, disabled }: ChooseFeatureProps) => {
  ...
  return (
    <>
      <Button
        className={`border-primary-100 bg-primary-25 text-primary-600 text-xs font-semibold`}
        onClick={() => !disabled && setShowFeaturesModal(true)}>
        <RiAddLine className="mr-1 h-4 w-4" />
        {t("appDebug.operation.addFeature")}
      </Button>
      {showFeaturesModal && <FeatureModal onChange={onChange} />}
    </>
  )
}
