...
import ConfirmAddVar from "@/components/configuration/config-prompt/confirm-add-var"
import { getNewVar } from "@/utils/var"
import { varHighlightHTML } from "@/components/configuration/base/var-highlight"
...
const OpeningStatement: FC<OpeningStatementProps> = ({
  ...
  const [isFocus, { setTrue: didSetFocus, setFalse: setBlur }] =
    useBoolean(false)
  ...
  const handleConfirm = () => {
    ...
    const { features, setFeatures } = getState()
  ...
  const cancelAutoAddVar = () => {
    ...
    const newFeatures = produce(features, draft => {
      if (draft.opening) draft.opening.opening_statement = tempValue
    })
    setFeatures(newFeatures)
  ...
  const headerRight = !readonly ? (
    isFocus ? (
      <div className="flex items-center space-x-1">
        ...
        <Button size="small" onClick={handleConfirm} variant="primary">
      ...
      <OperationBtn
        type="edit"
        actionName={
          hasValue ? "" : (t("appDebug.openingStatement.writeOpener") as string)
        }
        onClick={handleEdit}
      />
    )
    ...
    return isFocus ? (
      <div>
        <div className="flex items-center py-2">
          <div className="flex shrink-0 space-x-0.5 text-xs font-medium leading-[18px] text-gray-500">
          ...
          <div className="bg-[#243, 244, 246] ml-3 h-px w-0 grow"></div>
        </div>
        <ReactSortable
          className="space-y-1"
          ...
          {tempSuggestedQuestions.map((question, index) => {
            return (
              <div
                className="group relative flex items-center rounded-lg border border-gray-200 pl-2.5 hover:border-gray-300 hover:bg-white"
                key={index}>
                <div className="handle flex h-4 w-4 cursor-grab items-center justify-center">
                  <svg
                    ...
                    <path
                      fillRule="evenodd"
                      clipRule="evenodd"
                      d="M1 2C1.55228 2 2 1.55228 2 1C2 0.447715 1.55228 0 1 0C0.447715 0 0 0.447715 0 1C0 1.55228 0.447715 2 1 2ZM1 6C1.55228 6 2 5.55228 2 5C2 4.44772 1.55228 4 1 4C0.447715 4 0 4.44772 0 5C0 5.55228 0.447715 6 1 6ZM6 1C6 1.55228 5.55228 2 5 2C4.44772 2 4 1.55228 4 1C4 0.447715 4.44772 0 5 0C5.55228 0 6 0.447715 6 1ZM5 6C5.55228 6 6 5.55228 6 5C6 4.44772 5.55228 4 5 4C4.44772 4 4 4.44772 4 5C4 5.55228 4.44772 6 5 6ZM2 9C2 9.55229 1.55228 10 1 10C0.447715 10 0 9.55229 0 9C0 8.44771 0.447715 8 1 8C1.55228 8 2 8.44771 2 9ZM5 10C5.55228 10 6 9.55229 6 9C6 8.44771 5.55228 8 5 8C4.44772 8 4 8.44771 4 9C4 9.55229 4.44772 10 5 10Z"
                      fill="#98A2B3"
                    />
                  </svg>
                </div>
                <input
                  ...
                  className={
                    "h-9 w-full grow cursor-pointer overflow-x-auto rounded-lg border-0 bg-transparent pl-1.5 pr-8 text-sm leading-9 text-gray-900 focus:outline-none"
                  }
                />

                <div
                  className="absolute right-1.5 top-1/2 block translate-y-[-50%] cursor-pointer rounded-md p-1 hover:bg-[#FEE4E2] hover:text-[#D92D20]"
                  onClick={() => {
                    setTempSuggestedQuestions(
        ...
        {tempSuggestedQuestions.length < MAX_QUESTION_NUM && (
          <div
            ...
            className="mt-1 flex h-9 cursor-pointer items-center gap-2 rounded-lg bg-gray-100 px-3 text-gray-400 hover:bg-gray-200">
      ...
      <div className="mt-1.5 flex flex-wrap">
        {notEmptyQuestions.map((question, index) => {
          return (
            <div
              key={index}
              className="shadow-xs mr-1 mt-1 max-w-full shrink-0 cursor-pointer items-center truncate rounded-lg border border-gray-200 bg-white px-2.5 text-[13px] font-normal leading-8 text-gray-900 last:mr-0">
              {question}
            </div>
          )
        })}
  ...
  return (
    <Panel
      className={cn(isShowConfirmAddVar && "h-[220px]", "!bg-gray-25 relative")}
      ...
      <div className="text-sm text-gray-700">
        {hasValue || (!hasValue && isFocus) ? (
            ...
            {isFocus ? (
              <div>
                <textarea
                  ...
                  className="w-full border-0 bg-transparent px-0 text-sm focus:outline-none"
                  placeholder={
                    t("appDebug.openingStatement.placeholder") as string
                  }></textarea>
              </div>
            ) : (
              <div
          ...
          <div className="pb-1 pt-2 text-xs text-gray-500">
