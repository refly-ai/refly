...
const ParamConfigContent = ({ onChange }: ParamConfigContentProps) => {
  ...
  const transferMethod = useMemo(() => {
    if (
      ...
      file?.image.transfer_methods.length === 2
    )
  ...
  const handleTransferMethodsChange = useCallback(
    (value: TransferMethod) => {
      const { features, setFeatures } = featuresStore!.getState()
      const newFeatures = produce(features, draft => {
        if (draft.file?.image) {
          if (value === TransferMethod.all)
            draft.file.image.transfer_methods = [
              TransferMethod.remote_url,
              TransferMethod.local_file,
            ]
          else draft.file.image.transfer_methods = [value]
        }
      })
  ...
  const handleLimitsChange = useCallback(
    (_key: string, value: number) => {
      ...
      const { features, setFeatures } = featuresStore!.getState()
      const newFeatures = produce(features, draft => {
        if (draft.file?.image) draft.file.image.number_limits = value
      })
      setFeatures(newFeatures)
      if (onChange) onChange(newFeatures)
    },
