...
const BlockInput: FC<IBlockInputProps> = ({
  ...
  const style = classNames({
    "block px-4 py-2 w-full h-full text-sm text-gray-900 outline-0 border-0 break-all":
      true,
  ...
  const textAreaContent = (
    <div
      className={classNames(
        readonly ? "max-h-[180px] pb-5" : "h-[180px]",
        "overflow-y-auto",
      )}
      ...
      {isEditing ? (
        <div className="h-full px-4 py-2">
          <textarea
            ...
            className={classNames(
              editAreaClassName,
              "block h-full w-full resize-none",
            )}
  ...
  return (
    <div
      className={classNames(
        "block-input w-full overflow-y-auto rounded-xl border-none bg-white",
      )}>
      {textAreaContent}
      {/* footer */}
      {!readonly && (
