...
import { replaceStringWithValues } from "@/components/configuration/prompt-value-panel"
...
export const useCheckPromptVariables = () => {
  ...
  const checkPromptVariables = useCallback(
    (promptVariablesConfig: {
      inputs: Inputs
      promptVariables: PromptVariable[]
    }) => {
      const { promptVariables, inputs } = promptVariablesConfig
      let hasEmptyInput = ""
      const requiredVars = promptVariables.filter(
...
export const useChat = (
  ...
  const conversationMessagesAbortControllerRef = useRef<AbortController | null>(
    null,
  )
  const suggestedQuestionsAbortControllerRef = useRef<AbortController | null>(
    null,
  )
  ...
  const getIntroduction = useCallback(
    (str: string) => {
      return replaceStringWithValues(
        str,
        promptVariablesConfig?.promptVariables || [],
        promptVariablesConfig?.inputs || {},
      )
    },
        ...
        },
        {
          ...
          async onCompleted(hasError?: boolean) {
            ...
            if (
              ...
              const newResponseItem = data.find(
                (item: any) => item.id === responseItem.id,
              )
          ...
          onFile(file) {
            ...
            if (lastThought)
              responseItem.agent_thoughts![
                responseItem.agent_thoughts!.length - 1
              ].message_files = [...(lastThought as any).message_files, file]
          ...
          onThought(thought) {
            ...
            if (response.agent_thoughts.length === 0) {
              ...
              if (lastThought.id === thought.id) {
                ...
                responseItem.agent_thoughts![
                  response.agent_thoughts.length - 1
                ] = thought
              } else {
          ...
          onNodeFinished: ({ data }) => {
            ...
            const currentIndex =
              responseItem.workflowProcess!.tracing!.findIndex(
