...
import { Edit02 } from "@/components/base/icons/src/vender/line/general"
import { Star06 } from "@/components/base/icons/src/vender/solid/shapes"
...
const ConfigPanel = () => {
  ...
  return (
    <div className="flex max-h-[80%] w-full max-w-[720px] flex-col">
      <div
        className={`grow overflow-y-auto rounded-xl ${showConfigPanelBeforeChat && "border-[0.5px] border-gray-100 shadow-lg"} ${!showConfigPanelBeforeChat && collapsed && "border border-indigo-100"} ${!showConfigPanelBeforeChat && !collapsed && "border-[0.5px] border-gray-100 shadow-lg"} `}>
        <div
          className={`bg-indigo-25 flex flex-wrap rounded-t-xl px-6 py-4 ${isMobile && "!px-4 !py-3"} `}>
          {showConfigPanelBeforeChat && (
            <>
      ...
      {showConfigPanelBeforeChat && (site || customConfig) && (
        <div className="mt-4 flex flex-wrap items-center justify-between py-2 text-xs text-gray-400">
          {site?.privacy_policy ? (
            <div
              className={`flex items-center ${isMobile && "w-full justify-end"}`}>
          ...
          {customConfig?.remove_webapp_brand ? null : (
            <div
              className={`flex items-center justify-end ${isMobile && "w-full"}`}>
