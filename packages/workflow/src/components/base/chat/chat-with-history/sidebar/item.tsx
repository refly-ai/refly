...
const Item: FC<ItemProps> = ({
  ...
  return (
    <div
      ref={ref}
      key={item.id}
      className={`group mb-0.5 flex cursor-pointer rounded-lg py-1.5 pl-3 pr-1.5 text-sm font-medium text-gray-700 last-of-type:mb-0 hover:bg-gray-50 ${currentConversationId === item.id && "text-primary-600 bg-primary-50"} `}
      onClick={() => onChangeConversation(item.id)}>
      <MessageDotsCircle
        className={`mr-2 mt-1 h-4 w-4 shrink-0 text-gray-400 ${currentConversationId === item.id && "text-primary-600"}`}
      />
      <div className="grow break-all py-0.5" title={item.name}>
        {item.name}
      </div>
      {item.id !== "" && (
