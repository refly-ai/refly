...
import type { ChatConfig, OnSend } from "../types"
...
import LogoAvatar from "@/components/base/logo/logo-embeded-chat-avatar"
      ...
      return (
        <>
          {!currentConversationId && (
            <div
              className={cn(
                "tablet:px-4 mx-auto w-full max-w-[720px]",
                isMobile && "px-4",
              )}>
              <div className="mb-6" />
              <ConfigPanel />
  ...
  return (
    <Chat
      ...
      chatContainerInnerClassName={cn(
        "mx-auto w-full max-w-[720px] tablet:px-4",
        isMobile && "px-4",
      )}
      ...
      answerIcon={
        isDify() ? <LogoAvatar className="relative shrink-0" /> : null
      }
      hideProcessDetail
