...
export const useEmbeddedChatbot = () => {
  ...
  const { data: appPinnedConversationData } = useSWR(
    ["appConversationData", isInstalledApp, appId, true],
  ...
  const inputsForms = useMemo(() => {
    return (appParams?.user_input_form || [])
      .filter(
        (item: any) =>
          item.paragraph || item.select || item["text-input"] || item.number,
      )
      .map((item: any) => {
        if (item.paragraph) {
          return {
