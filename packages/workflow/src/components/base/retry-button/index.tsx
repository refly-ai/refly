...
const RetryButton: FC<Props> = ({ datasetId }) => {
  ...
  const [indexState, dispatch] = useReducer(indexStateReducer, {
    value: "success",
  })
  ...
  const onRetryErrorDocs = async () => {
    dispatch({ type: "retry" })
    const document_ids =
      errorDocs?.data.map((doc: IndexingStatusResponse) => doc.id) || []
    const res = await retryErrorDocs({ datasetId, document_ids })
    if (res.result === "success") dispatch({ type: "success" })
  ...
  if (indexState.value === "success") return null
  ...
  return (
    <div
      ...
      <span
        className={classNames(
          "text-primary-600 cursor-pointer text-sm font-semibold",
          indexState.value === "retry" && "!cursor-not-allowed !text-gray-500",
        )}
