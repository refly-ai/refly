...
const VoiceInput = ({ onCancel, onConverted }: VoiceInputTypes) => {
  ...
  return (
    <div className={cn(s.wrapper, "absolute inset-0 rounded-xl")}>
      <div className="bg-primary-25 absolute inset-[1.5px] flex items-center overflow-hidden rounded-[10.5px] py-[14px] pl-[14.5px] pr-[6.5px]">
        <canvas
          id="voice-input-record"
          className="absolute bottom-0 left-0 h-4 w-full"
        />
        {startConvert && (
          <RiLoader2Line className="text-primary-700 mr-2 h-4 w-4 animate-spin" />
        )}
        <div className="grow">
          {startRecord && (
        ...
        {startRecord && (
          <div
            className="hover:bg-primary-100 mr-1 flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg"
            onClick={handleStopRecorder}>
            <StopCircle className="text-primary-600 h-5 w-5" />
          </div>
        )}
        {startConvert && (
          <div
        ...
        <div
          className={`w-[45px] pl-1 text-xs font-medium ${originDuration > 110 ? "text-[#F04438]" : "text-gray-700"}`}>{`0${minutes.toFixed(0)}:${seconds >= 10 ? seconds : `0${seconds}`}`}</div>
      </div>
    </div>
  )
}
