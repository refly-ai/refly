...
const AudioBtn = ({
  ...
  const removeCodeBlocks = (inputText: any) => {
    ...
    if (inputText) return inputText.replace(codeBlockRegex, "")
    return ""
  }
  ...
  return (
    <div
      className={`${audioState === "loading" || audioState === "playing" ? "mr-1" : className}`}>
      <Tooltip
        ...
        <button
          ...
          {audioState === "loading" ? (
            ...
            <div
              className={`h-6 w-6 rounded-md ${!isAudition ? "h-4 w-4 hover:bg-gray-50" : "hover:bg-gray-50"} ${audioState === "playing" ? s.pauseIcon : s.playIcon}`}></div>
          )}
        </button>
      </Tooltip>
