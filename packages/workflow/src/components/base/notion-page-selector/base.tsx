...
const NotionPageSelector = ({
  ...
  const { data, mutate } = useSWR(
    { url: "/notion/pre-import/pages", datasetId },
  ...
  const currentWorkspace = notionWorkspaces.find(
    workspace => workspace.workspace_id === currentWorkspaceId,
  )

  const getPagesMapAndSelectedPagesId: [
    DataSourceNotionPageMap,
    Set<string>,
    Set<string>,
  ] = useMemo(() => {
    ...
    const pagesMap = notionWorkspaces.reduce(
  ...
  const defaultSelectedPagesId = [
    ...Array.from(getPagesMapAndSelectedPagesId[1]),
    ...(value || []),
  ]
  const [selectedPagesId, setSelectedPagesId] = useState<Set<string>>(
    new Set(defaultSelectedPagesId),
  )
  ...
  const handleSelecPages = (newSelectedPagesId: Set<string>) => {
    const selectedPages = Array.from(newSelectedPagesId).map(
      pageId => getPagesMapAndSelectedPagesId[0][pageId],
    )
  ...
  const handlePreviewPage = (previewPageId: string) => {
    if (onPreview) onPreview(getPagesMapAndSelectedPagesId[0][previewPageId])
  }
