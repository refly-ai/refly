  ...
  return (
    <PortalToFollowElem
      ...
      <PortalToFollowElemTrigger onClick={handleToggle}>
        <div
          className={`relative flex h-8 items-center justify-center rounded-lg bg-gray-100 px-3 text-xs text-gray-500 hover:bg-gray-200 ${disabled ? "cursor-not-allowed" : "cursor-pointer"} `}>
          <Link03 className="mr-2 h-4 w-4" />
          {t("common.imageUploader.pasteImageLink")}
        </div>
      </PortalToFollowElemTrigger>
      <PortalToFollowElemContent className="z-10">
        <div className="w-[320px] rounded-lg border-[0.5px] border-gray-200 bg-white p-2 shadow-lg">
...
const TextGenerationImageUploader: FC<TextGenerationImageUploaderProps> = ({
  ...
  const localUpload = (
    <Uploader
      ...
      {hovering => (
        <div
          className={`flex h-8 cursor-pointer items-center justify-center rounded-lg bg-gray-100 px-3 text-xs text-gray-500 ${hovering && "bg-gray-200"} `}>
      ...
      <div
        ...
        {settings.transfer_methods.map(method => {
          if (method === TransferMethod.local_file)
            return (
              <Fragment key={TransferMethod.local_file}>{localUpload}</Fragment>
            )

          if (method === TransferMethod.remote_url)
            return (
              <Fragment key={TransferMethod.remote_url}>{urlUpload}</Fragment>
            )
