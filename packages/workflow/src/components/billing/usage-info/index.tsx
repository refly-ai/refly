...
const UsageInfo: FC<Props> = ({
  ...
  const color = (() => {
    if (percent < LOW) return "#155EEF"

    if (percent < MIDDLE) return "#F79009"
    ...
    <div className={className}>
      <div className="flex h-5 items-center justify-between">
        <div className="flex items-center">
          ...
          <div className="mx-1 text-sm font-medium leading-5 text-gray-700">
            {name}
          </div>
          {tooltip && (
            <Tooltip
              htmlContent={<div className="w-[180px]">{tooltip}</div>}
              selector="config-var-tooltip">
              <InfoCircle className="h-[14px] w-[14px] text-gray-400" />
            </Tooltip>
          )}
        </div>
