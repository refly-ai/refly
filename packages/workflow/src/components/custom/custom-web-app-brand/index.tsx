...
import { imageUpload } from "@/components/base/image-uploader/utils"
  ...
  return (
    <div className="py-4">
      ...
      <div className="border-black/8 shadow-xs relative mb-4 overflow-hidden rounded-xl border-[0.5px] bg-gray-50 pb-6 pl-4 pr-[119px]">
        <div
          className={`${s.mask} absolute -bottom-2 left-0 top-0 z-10 w-full`}></div>
      ...
      <div
        className={`flex items-center justify-between rounded-xl border-[0.5px] border-gray-200 bg-gray-50 px-4 py-3 ${webappBrandRemoved && "opacity-30"} `}>
        <div>
        ...
        <div className="flex items-center">
          {!uploading && (
            <Button className={`relative mr-2`} disabled={uploadDisabled}>
              ...
              <input
                className={`absolute inset-0 block w-full text-[0] opacity-0 ${uploadDisabled ? "cursor-not-allowed" : "cursor-pointer"} `}
