  ...
  return (
    <div className="">
      <Menu as="div" className="relative inline-block text-left">
        <div>
          <Menu.Button className="inline-flex h-7 w-full items-center justify-center rounded-[10px] pl-2 pr-2.5 text-[14px] font-semibold text-[#1C64F2] hover:bg-[#EBF5FF]">
            {curApp?.name}
                ...
                {appItems.map((app: AppDetailResponse) => (
                  <Menu.Item key={app.id}>
                    <div
                      ...
                      onClick={() =>
                        router.push(
                          `/app/${app.id}/${isCurrentWorkspaceEditor ? "configuration" : "overview"}`,
                        )
                      }>
              ...
              <Menu.Item>
                <div className="p-1" onClick={() => setShowNewAppDialog(true)}>
                  <div className="flex h-12 cursor-pointer items-center rounded-lg hover:bg-gray-100">
                    <div className="ml-4 mr-2 flex h-6 w-6 items-center justify-center rounded-[6px] border-[0.5px] border-dashed border-gray-200 bg-gray-100">
