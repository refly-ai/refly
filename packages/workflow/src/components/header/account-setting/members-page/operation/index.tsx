...
const Operation = ({ member, onOperate }: IOperationProps) => {
  ...
  const handleDeleteMemberOrCancelInvitation = async () => {
    try {
      await deleteMemberOrCancelInvitation({
    ...
    <Menu as="div" className="relative h-full w-full">
      {({ open }) => (
        <>
          <Menu.Button
            className={cn(
              `group flex h-full w-full cursor-pointer items-center justify-between hover:bg-gray-100 ${open && "bg-gray-100"} px-3 text-[13px] text-gray-700`,
            )}>
            {RoleMap[member.role] || RoleMap.normal}
            <ChevronDownIcon
              className={`h-4 w-4 group-hover:block ${open ? "block" : "hidden"}`}
            />
          </Menu.Button>
          <Transition
            as={Fragment}
            ...
            <Menu.Items
              className={cn(
                `absolute right-0 top-[52px] z-10 origin-top-right divide-y divide-gray-100 rounded-lg border-[0.5px] border-gray-200 bg-white`,
                s.popup,
              )}>
                  ...
                  <Menu.Item key={role}>
                    <div
                      ...
                      <div>
                        ...
                        <div className={itemDescClassName}>
