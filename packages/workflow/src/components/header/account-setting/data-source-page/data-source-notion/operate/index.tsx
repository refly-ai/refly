...
export default function Operate({ payload, onAuthAgain }: OperateProps) {
  const itemClassName = `
  ...
  const handleSync = async () => {
    await syncDataSourceNotion({
      url: `/oauth/data-source/notion/${payload.id}/sync`,
    })
    updateIntegrates()
  }
  const handleRemove = async () => {
    await updateDataSourceNotionAction({
      url: `/data-source/integrates/${payload.id}/disable`,
    })
    updateIntegrates()
  }
  ...
  return (
    <Menu as="div" className="relative inline-block text-left">
      {({ open }) => (
        <>
          <Menu.Button
            className={`flex h-8 w-8 items-center justify-center rounded-lg hover:bg-gray-100 ${open && "bg-gray-100"}`}>
          ...
          <Transition
            ...
            <Menu.Items className="max-w-80 absolute right-0 top-9 w-60 origin-top-right divide-y divide-gray-100 rounded-lg bg-white shadow-lg">
              <div className="px-1 py-1">
                <Menu.Item>
                  <div className={itemClassName} onClick={onAuthAgain}>
