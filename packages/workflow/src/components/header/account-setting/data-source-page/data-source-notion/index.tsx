...
const Icon: FC<{
  ...
  return <NotionIcon src={src} name={name} className={className} />
}
type Props = {
  workspaces: TDataSourceNotion[]
}

const DataSourceNotion: FC<Props> = ({ workspaces }) => {
  const { isCurrentWorkspaceManager } = useAppContext()
  const [canConnectNotion, setCanConnectNotion] = useState(false)
  const { data } = useSWR(
    canConnectNotion ? "/oauth/data-source/notion" : null,
  ...
  const handleConnectNotion = () => {
    if (!isCurrentWorkspaceManager) return
