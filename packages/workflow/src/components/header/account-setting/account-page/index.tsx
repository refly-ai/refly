  ...
  const renderAppItem = (item: IItem) => {
    return (
      <div className="flex px-3 py-1">
        ...
        <div className="mt-[3px] text-xs font-medium leading-[18px] text-gray-700">
          {item.name}
        </div>
      </div>
    )
  ...
  return (
    <>
      ...
      <div className="mb-8">
        ...
        <div
          className={classNames(
            "group mt-2 flex h-9 w-full cursor-pointer items-center justify-between rounded bg-gray-100 px-3 text-sm font-normal text-gray-800",
          )}>
          {userProfile.name}
          <div
            className="hidden h-6 items-center rounded-md border border-gray-200 bg-white px-2 text-xs font-normal group-hover:flex"
      ...
      <div className="mb-8">
        ...
        <div className={classNames(inputClassName, "cursor-pointer")}>
          {userProfile.email}
        </div>
      </div>
      {IS_CE_EDITION && (
        <div className="mb-8">
          ...
          <Button onClick={() => setEditPasswordModalVisible(true)}>
            {userProfile.is_password_set
              ? t("common.account.resetPassword")
              : t("common.account.setPassword")}
          </Button>
        </div>
      )}
      ...
      {editNameModalVisible && (
        <Modal
          ...
          <div className="mt-10 flex justify-end">
            <Button
              className="mr-2"
              onClick={() => setEditNameModalVisible(false)}>
      ...
      {showDeleteAccountModal && (
        <Modal
          className={classNames("w-[480px] max-w-[480px] p-8", s.bg)}
          ...
          <div className="h-12 w-12 rounded-xl border-[0.5px] border-gray-100 bg-white p-3 shadow-xl">
            <RiErrorWarningFill className="h-6 w-6 text-[#D92D20]" />
          </div>
          <div className="relative mt-3 text-xl font-semibold leading-[30px] text-gray-900">
            {t("common.account.delete")}
          </div>
          <div className="my-1 text-sm leading-5 text-[#D92D20]">
          ...
          <div className="my-2 rounded-lg bg-gray-100 px-3 py-2 text-sm font-medium leading-5 text-gray-800">{`Delete Account: ${userProfile.email}`}</div>
