...
import { useLanguage, useProviderCredentialsAndLoadBalancing } from "../hooks"
...
import ConfirmCommon from "@/components/base/confirm/common"
...
const ModelModal: FC<ModelModalProps> = ({
  ...
  const originalConfigMap = useMemo(() => {
    ...
    return originalConfig?.configs.reduce(
      (prev, config) => {
  ...
  useEffect(() => {
    if (originalConfig && !draftConfig) setDraftConfig(originalConfig)
  ...
  const [requiredFormSchemas, defaultFormSchemaValue, showOnVariableMap] =
    useMemo(() => {
      ...
      formSchemas.forEach(formSchema => {
        if (formSchema.required) requiredFormSchemas.push(formSchema)
        ...
        if (formSchema.show_on.length) {
          formSchema.show_on.forEach(showOnItem => {
            ...
            if (
              !showOnVariableMap[showOnItem.variable].includes(
                formSchema.variable,
              )
            )
        ...
        if (
          ...
          ;(
            formSchema as CredentialFormSchemaRadio | CredentialFormSchemaSelect
            ...
            if (option.show_on.length) {
              option.show_on.forEach(showOnItem => {
                ...
                if (
                  ...
                  showOnVariableMap[showOnItem.variable].push(
                    formSchema.variable,
                  )
              })
            }
      ...
      return [requiredFormSchemas, defaultFormSchemaValue, showOnVariableMap]
    }, [formSchemas])
  const initialFormSchemasValue: Record<string, string | number> =
    useMemo(() => {
      return {
  ...
  const filteredRequiredFormSchemas = requiredFormSchemas.filter(
    requiredFormSchema => {
      if (
        requiredFormSchema.show_on.length &&
        requiredFormSchema.show_on.every(
          showOnItem => value[showOnItem.variable] === showOnItem.value,
        )
      )
        return true

      if (!requiredFormSchema.show_on.length) return true
  ...
  const encodeSecretValues = useCallback(
    (v: FormValue) => {
      const result = { ...v }
      extendedSecretFormSchemas.forEach(({ variable }) => {
        if (
          result[variable] === formSchemasValue?.[variable] &&
  ...
  const encodeConfigEntrySecretValues = useCallback(
    (entry: ModelLoadBalancingConfigEntry) => {
      const result = { ...entry }
      extendedSecretFormSchemas.forEach(({ variable }) => {
        if (
          entry.id &&
          result.credentials[variable] ===
            originalConfigMap[entry.id]?.credentials?.[variable]
        )
    ...
    try {
      ...
      const res = await saveCredentials(
        ...
        encodeSecretValues(value),
        {
          ...
          configs:
            draftConfig?.configs.map(encodeConfigEntrySecretValues) || [],
        },
      )
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/[.25]">
          <div className="mx-2 max-h-[calc(100vh-120px)] w-[640px] overflow-y-auto rounded-2xl bg-white shadow-xl">
            <div className="px-8 pt-8">
              ...
              <div className="mb-4 mt-1 border-t-[0.5px] border-t-gray-100" />
              ...
              <div className="sticky bottom-0 -mx-2 mt-2 flex flex-wrap items-center justify-between gap-y-2 bg-white px-2 pb-6 pt-4">
                {provider.help && (provider.help.title || provider.help.url) ? (
                  <a
                    href={
                      provider.help?.url[language] || provider.help?.url.en_US
                    }
                    target="_blank"
                ...
                <div>
                  ...
                  <Button size="large" className="mr-2" onClick={onCancel}>
                  ...
                  <Button
                    ...
                    disabled={
                      loading ||
                      filteredRequiredFormSchemas.some(
                        item => value[item.variable] === undefined,
                      ) ||
                      (draftConfig?.enabled &&
                        (draftConfig?.configs.filter(config => config.enabled)
                          .length ?? 0) < 2)
                    }>
