      ...
      <span className="ml-1 pt-1.5">
        <Tooltip
          popupContent={
            ...
            <div className="">{tooltip[language] || tooltip.en_US}</div>
          }>
          <RiQuestionLine className="h-3 w-3 text-gray-500" />
        </Tooltip>
      </span>
    )
    if (
      ...
      return (
        <div key={variable} className={cn(itemClassName, "py-3")}>
          <div
            className={cn(fieldLabelClassName, "py-2 text-sm text-gray-900")}>
        ...
        <div key={variable} className={cn(itemClassName, "py-3")}>
          ...
          <div className={`grid grid-cols-${options?.length} gap-3`}>
            {options
              ...
              .map(option => (
                <div
                  className={`bg-gray-25 flex cursor-pointer items-center rounded-lg border border-gray-100 px-3 py-2 ${value[variable] === option.value && "border-primary-400 border-[1.5px] bg-white shadow-sm"} ${disabed && "!cursor-not-allowed opacity-60"} `}
                  onClick={() => handleFormChange(variable, option.value)}
                  key={`${variable}-${option.value}`}>
                  <div
                    className={`mr-2 flex h-4 w-4 items-center justify-center rounded-full border border-gray-300 ${value[variable] === option.value && "border-primary-600 border-[5px]"} `}
                  />
