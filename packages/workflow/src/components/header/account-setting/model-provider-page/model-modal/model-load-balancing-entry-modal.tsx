...
const ModelLoadBalancingEntryModal: FC<ModelModalProps> = ({
  ...
  const [initialValue, setInitialValue] =
    useState<ModelLoadBalancingConfigEntry["credentials"]>()
  useEffect(() => {
  ...
  const getSecretValues = useCallback(
    (v: FormValue) => {
      return secretFormSchemas.reduce(
        (prev, next) => {
    ...
    try {
      ...
      if (res.status === ValidatedStatus.Success) {
        ...
        onSave({
          ...(entry || {}),
          name: name as string,
          credentials: credentials as Record<
            string,
            ...
            <div className="px-8 pt-8">
              <div className="mb-2 flex items-center justify-between">
                <div className="text-xl font-semibold text-gray-900">
                  {t(
                    isEditMode
                      ? "common.modelProvider.editConfig"
                      : "common.modelProvider.addConfig",
                  )}
                </div>
              </div>
              <Form
