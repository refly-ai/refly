...
const CredentialPanel: FC<CredentialPanelProps> = ({ provider, onSetup }) => {
  ...
  return (
    <>
      {provider.provider_credential_schema && (
        <div className="relative ml-1 w-[112px] shrink-0 rounded-lg border-[0.5px] border-black/5 bg-white/[0.3] p-1">
          <div className="mb-1 flex h-5 items-center justify-between pl-2 pr-[7px] pt-1 text-xs font-medium text-gray-500">
            API-KEY
            <Indicator color={customConfiged ? "green" : "gray"} />
          </div>
          <div className="flex items-center gap-0.5">
            <Button className="grow" size="small" onClick={onSetup}>
              <Settings01 className="mr-1 h-3 w-3" />
          ...
          {priorityUseType === PreferredProviderTypeEnum.custom &&
            systemConfig.enabled && <PriorityUseTip />}
        </div>
      )}
