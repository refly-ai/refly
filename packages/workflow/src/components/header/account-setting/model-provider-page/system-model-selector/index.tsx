...
const SystemModel: FC<SystemModelSelectorProps> = ({
  ...
  const [currentTTSDefaultModel, changeCurrentTTSDefaultModel] =
  ...
  const handleChangeDefaultModel = (
    ...
    if (res.result === "success") {
      ...
      changedModelTypes.forEach(modelType => {
        if (modelType === ModelTypeEnum.textGeneration)
          ...
          updateModelList(modelType)
        else if (modelType === ModelTypeEnum.rerank) updateModelList(modelType)
        else if (modelType === ModelTypeEnum.speech2text)
          updateModelList(modelType)
        else if (modelType === ModelTypeEnum.tts) updateModelList(modelType)
      })
    }
  }
  ...
  return (
    <PortalToFollowElem
      ...
      <PortalToFollowElemTrigger onClick={() => setOpen(v => !v)}>
        <div
          className={`shadow-xs flex h-6 cursor-pointer items-center rounded-md border-[0.5px] border-gray-200 bg-white px-2 text-xs text-gray-700 hover:bg-gray-100 hover:shadow-none ${open && "bg-gray-100 shadow-none"} `}>
          <Settings01 className="mr-1 h-3 w-3 text-gray-500" />
          {t("common.modelProvider.systemModelSettings")}
        </div>
      </PortalToFollowElemTrigger>
      <PortalToFollowElemContent className="z-50">
        <div className="w-[360px] rounded-xl border-[0.5px] border-black/5 bg-white pt-4 shadow-xl">
          <div className="px-6 py-1">
            <div className="flex h-8 items-center text-[13px] font-medium text-gray-900">
              ...
              <Tooltip
                ...
                <RiQuestionLine className="ml-0.5 h-[14px] w-[14px] text-gray-400" />
              </Tooltip>
            </div>
            <div>
              <ModelSelector
