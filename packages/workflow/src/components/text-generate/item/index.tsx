...
import { File02 } from "@/components/base/icons/src/vender/line/files"
import { Bookmark } from "@/components/base/icons/src/vender/line/general"
import { Stars02 } from "@/components/base/icons/src/vender/line/weather"
...
import WorkflowProcessItem from "@/components/base/chat/chat/answer/workflow-process"
  ...
  <div
    className={cn(
      ...
      isDisabled
        ...
        : "cursor-pointer border-gray-200 text-gray-700 hover:border-gray-300 hover:shadow-sm",
      "flex h-7 items-center rounded-md border px-3 text-xs font-medium",
    )}
...
const GenerationItem: FC<IGenerationItemProps> = ({
  ...
  const question =
    varList && varList?.length > 0
      ? varList?.map(({ label, value }) => `${label}:${value}`).join("&")
      : ""
  const [isQuerying, { setTrue: startQuerying, setFalse: stopQuerying }] =
    useBoolean(false)
  ...
  const handleMoreLikeThis = async () => {
    ...
    startQuerying()
    const res: any = await fetchMoreLikeThis(
      messageId as string,
      isInstalledApp,
      installedAppId,
    )
  ...
  const mainStyle = (() => {
    ...
    if (hasChild) res.boxShadow = "0px 1px 2px rgba(16, 24, 40, 0.05)"
  ...
  return (
    <div
      ...
      className={cn(
        className,
        isTop
          ? `rounded-xl border ${!isError ? "border-gray-200 bg-white" : "border-[#FECDCA] bg-[#FEF3F2]"} `
          : "!mt-0 rounded-br-xl",
      )}
      style={
        isTop
      ...
      {isLoading ? (
        ...
        <div
          className={cn(
            !isTop && "border-primary-400 rounded-br-xl border-l-2",
            "p-4",
            innerClassName,
          )}
          style={mainStyle}>
          ...
          <div className="mt-3 flex items-center justify-between">
            <div className="flex items-center">
                ...
                <>
                  ...
                  {moreLikeThis && depth < MAX_DEPTH && (
                    <SimpleBtn
                      ...
                      {!isMobile && (
                        <div>{t("appDebug.feature.moreLikeThis.title")}</div>
                      )}
                    </SimpleBtn>
                  )}
                  {isError && (
                    <SimpleBtn
                      ...
                      {!isMobile && (
                        <div>{t("share.generation.batchFailed.retry")}</div>
                      )}
                    </SimpleBtn>
                  )}
              ...
              {supportFeedback && <div className="ml-1">{ratingContent}</div>}
