...
import { Tag01 } from "@/components/base/icons/src/vender/line/financeAndECommerce"
import type { ToolWithProvider } from "@/components/workflow/types"
...
import { useStore as useLabelStore } from "@/components/tools/labels/store"
...
const Blocks = ({
  ...
  const renderGroup = useCallback(
    (toolWithProvider: ToolWithProvider) => {
      ...
      const needAuth =
        toolWithProvider.allow_delete &&
        !toolWithProvider.is_team_authorization &&
        toolWithProvider.type === CollectionType.builtIn

      return (
        <div key={toolWithProvider.id} className="group mb-1 last-of-type:mb-0">
          <div className="flex h-[22px] w-full items-center justify-between pl-3 pr-1 text-xs font-medium text-gray-500">
            ...
            <a
              className="hidden cursor-pointer items-center group-hover:flex"
              href={`/tools?category=${toolWithProvider.type}`}
              target="_blank">
              {t("tools.addToolModal.manageInTools")}
              <ArrowUpRight className="ml-0.5 h-3 w-3" />
            </a>
          </div>
          {list.map(tool => {
            const labelContent = (() => {
              if (!tool.labels) return ""
              return tool.labels
                .map(name => {
            ...
            const added = !!addedTools?.find(
              v =>
                v.provider_id === toolWithProvider.id &&
                v.provider_type === toolWithProvider.type &&
                v.tool_name === tool.name,
            )
            return (
              <Tooltip
                key={tool.name}
                ...
                position="bottom"
                className="!w-[210px] translate-x-[108px] !rounded-xl !border-[0.5px] !border-black/5 !bg-transparent !p-0 !px-3 !py-2.5 !text-xs !leading-[18px] !text-gray-700 !shadow-lg"
                htmlContent={
                  <div>
                    <BlockIcon
                      size="md"
                    ...
                    {tool.labels?.length > 0 && (
                      <div className="mt-1 flex shrink-0 items-center">
                        <div
                          className="relative flex w-full items-center gap-1 rounded-md py-1 text-gray-500"
                          title={labelContent}>
                          <Tag01 className="h-3 w-3 shrink-0 text-gray-500" />
                          <div className="grow truncate text-start text-xs font-normal leading-[18px]">
                            {labelContent}
                          </div>
                        </div>
                      </div>
                ...
                <div className="group/item flex h-8 w-full cursor-pointer items-center rounded-lg pl-3 pr-1 hover:bg-gray-50">
                  ...
                  <div
                    className={cn(
                      "grow truncate text-sm text-gray-900",
                      needAuth && "opacity-30",
                    )}>
                    {tool.label[language]}
                  </div>
                  ...
                  {!needAuth && !added && addable && (
                    <Button
                      ...
                      size="small"
                      className={cn(
                        "hidden shrink-0 items-center group-hover/item:flex",
                      )}
