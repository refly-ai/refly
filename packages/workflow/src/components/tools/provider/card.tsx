...
const ProviderCard = ({ active, collection, onSelect }: Props) => {
  ...
  return (
    <div
      className={cn(
        "group col-span-1 flex min-h-[160px] cursor-pointer flex-col rounded-xl border-2 border-solid border-transparent bg-white shadow-sm transition-all duration-200 ease-in-out hover:shadow-lg",
      ...
      <div
        className={cn(
          ...
          collection.labels?.length > 0 &&
            "group-hover:line-clamp-2 group-hover:max-h-[36px]",
        )}
      ...
      {collection.labels?.length > 0 && (
        <div className="mt-1 flex h-[42px] shrink-0 items-center pb-[6px] pl-[14px] pr-[6px] pt-1">
          <div
            className="relative flex w-full items-center gap-1 rounded-md py-[7px] text-gray-500"
            title={labelContent}>
