...
import type { ConfirmCommonProps } from "@/components/base/confirm/common"
import Confirm from "@/components/base/confirm/common"
...
export const useCheckFreeQuota = () => {
  ...
  useEffect(() => {
    if (error) router.replace("/", { forceOptimisticNavigation: false })
  }, [error, router])

  useEffect(() => {
    if (
      type === "provider_apply_callback" &&
      FREE_CHECK_PROVIDER.includes(provider as string) &&
      result === "success"
    )
  ...
  return data && provider
    ? {
        ...
        title: data.flag
          ? QUOTA_RECEIVE_STATUS[provider as string].success[locale]
          : QUOTA_RECEIVE_STATUS[provider].fail[locale],
...
export const useCheckNotion = () => {
  ...
  useEffect(() => {
    if (data) router.replace("/", { forceOptimisticNavigation: false })
  }, [data, router])
  useEffect(() => {
    if (type === "notion") {
      if (notionError) {
...
export const CheckModal = () => {
  ...
  if (!confirmInfo || !showPayStatusModal) return null
  ...
  return (
    <Confirm
      ...
      confirmText={
        (confirmInfo.type === "danger" && t("common.operation.ok")) || ""
      }
    />
  )
}
