...
export class ShareCreationService {
  ...
  private sanitizeCanvasDataForPublic(
    ...
    // 2. Sanitize each node - keep only safe, public fields
    const sanitizedNodes = resultNodes.map((node) => ({
      id: node.id,
      // Remove type to avoid revealing specific skill/node types used in workflow
      data: {
        entityId: node.data?.entityId || '',
    ...
    // 4. Filter and sanitize files - only result-related files without internal keys
    const sanitizedFiles = (canvasData.files || [])
      .filter((file) => file.resultId && resultEntityIds.has(file.resultId))
      .map((file) => {
        // Keep only essential fields for file display and access
        return {
          fileId: file.fileId,
          name: file.name,
          type: file.type,
          ...
          // - category/source/scope: reveal workflow methodology
