...
| `opt-in` | PTC on only for nodes whose title contains "ptc" |
...
PTC_MODE=on/partial and user allowed
  → ptcEnabled = true
  → if PTC_DEBUG=opt-in:  ptcEnabled = title.includes("ptc")
...
- Title matching is case-insensitive (`toLowerCase()`). "ptc" matches "PTC-test", "myPtcNode", etc.
