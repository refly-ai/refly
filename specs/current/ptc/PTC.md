...
| `opt-in` | PTC on only for nodes whose title contains `"ptc"` (case-insensitive) |
...
#### 4. Local Dev — Opt-in Debug (only nodes titled "ptc" trigger PTC)
```env
PTC_MODE=on
PTC_DEBUG=opt-in
```

#### 5. Local Dev — Opt-out Debug (all nodes except those titled "nonptc" trigger PTC)
```env
PTC_MODE=on
PTC_DEBUG=opt-out
```
