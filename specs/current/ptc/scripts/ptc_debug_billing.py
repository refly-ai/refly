...
def print_billing_trace(
    ...
    row = cur.fetchone()
    tool_credits = model_credits = total_credits = tool_original = 0
    if row:
    ...
    print(DIVIDER)
    print(
        f"  Tools:  {tool_credits:>8} credits  (agent: {total_agent_credits} | ptc: {total_ptc_credits})"
    )
    print(f"  Model:  {model_credits:>8} credits")
    if total_discount > 0:
        print(
            f"  Total:  {total_credits:>8} credits  (original: {tool_original}, discount: {total_discount})"
        )
    else:
...
def print_warnings(warnings, section_num, full=False):
    if not warnings:
        return
    print(
        f"[{section_num}] ⚠️  BILLING WARNINGS ({len(warnings)} issue{'s' if len(warnings) != 1 else ''})"
    )
    print()
...
def main():
    parser = make_arg_parser(
        "PTC Debug Billing: full billing trace for an agent result"
    )
